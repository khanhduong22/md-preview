# Project Master Overview: Organic Flow
...
flowchart LR
    ...
    subgraph System ["Các tính năng chính (Organic Flow)"]
        direction TB
        UC1(["Tạo & Quản lý Profile (User-Agent, Options)"])
        UC2(["Cấu hình Proxy & API Đổi IP"])
        UC3(["Cấu hình kịch bản (Keyword, Domain)"])
        UC4(["Mở / Đóng Browser thủ công"])
        UC5(["Chạy tiến trình Automation (SEO Auto)"])
...
flowchart TB
    ...
    subgraph App ["📦 Electron Desktop App"]
        ...
        Engine["🚀 Browser Engine - Playwright + Stealth Plugin"]:::engine
    ...
    subgraph Externals ["🌐 Môi trường Internet"]
        Proxy["🛡️ Proxy Providers - HTTP/Auth hoặc API VN"]:::external
        Google["🔍 Google Search - Bot Detection, CAPTCHA"]:::external
        Target["🎯 Target Website - Bypass GA4 / Tăng Traffic"]:::external
    end

    User -->|Thao tac| UI
    Engine -->|Dinh tuyen| Proxy
    Proxy -->|Mo phong| Google
    Proxy -->|Click Luot| Target
```
...
erDiagram
    PROFILE {
        ...
        String proxy "HTTP Proxy hoặc API lấy IP (Tuỳ chọn)"
...
flowchart TD
    ...
    subgraph Launching ["🚀 Khởi tạo Profile (browserEngine.ts)"]
        L1("Resolve Proxy (API / Auth)"):::browser
...
sequenceDiagram
    ...
    opt Nếu Browser chưa được khởi chạy
        ...
        IPC->>DB: Truy vấn thông tin profile (Proxy, UserAgent)
        DB-->>IPC: Trả về Profile Config
        IPC->>BE: Gọi `launchProfile()`

        Note over BE: Xử lý Proxy (Dynamic API/Auth)<br/>Khởi chạy Chromium Persistent Context
    ...
    alt Dính Google CAPTCHA
        BE->>BE: Gọi `clearGoogleCookies()` xoá dấu vết
        BE-->>IPC: Báo lỗi "CAPTCHA Detected" & Ảnh chụp
...
stateDiagram-v2
    ...
    Initializing --> BrowserLaunched: Load Proxy & Mở Chrome

    BrowserLaunched --> NavigatingGoogle: Gọi startAutomation

    NavigatingGoogle --> CaptchaDetected: Dính Captcha
    NavigatingGoogle --> TypingKeyword: Pass Captcha (Bình thường)

    TypingKeyword --> Searching: Gõ xong & Enter
    Searching --> CaptchaDetected: Check Captcha lần 2
    ...
    CaptchaDetected --> Failed: Xóa Cookies & Dừng
...
flowchart TD
    ...
    subgraph OS ["💻 Máy tính Khách hàng (Windows / macOS)"]
        direction TB
        App["📦 OrganicFlow App (App.exe / App.dmg)"]:::software
        ...
        Playwright["🌐 Trình duyệt Chromium (Tải ngầm Playwright)"]:::software

        App -->|Đọc / Ghi cấu hình| LocalDB
        App -->|Khởi chạy| Playwright
        Playwright -->|Lưu state| ProfileData
    ...
    subgraph Network ["🌍 Network & Security"]
        Proxy["🛡️ Proxy / VPN Server (Thay đổi IP Public)"]:::hardware
    end
    ...
    Playwright <-->|Traffic HTTPS| Proxy
    Proxy <-->|Bypass Bot Detect| Google
    Proxy <-->|Bypass GA4| Target
```
...
```mermaid
journey
    title Trải nghiệm sử dụng Organic Flow của Khách hàng

    section 1. Thiết lập Hệ thống (Onboarding)
      Tải & Cài đặt App: 5: User
      Nhập Key Proxy / API Đổi IP: 4: User, System
    ...
    section 3. Thực thi & Tự động hoá
      ...
      Chờ Bot vượt Google Captcha: 3: System
...
```mermaid
mindmap
  root((Organic Flow<br/>Core Product))
    Quản lý Profile
      Tạo / Sửa / Xóa
      Nhập xuất (Import/Export)
      Cấu hình Fingerprint
    Quản lý Proxy
      Hỗ trợ Proxy Tĩnh (Auth)
      Hỗ trợ Proxy API (Xoay IP)
      Tự động Anonymize
...
gitGraph
    ...
    commit id: "Tích hợp Playwright"
    commit id: "Xử lý Bypass Captcha"
...
```mermaid
gantt
    title Lộ trình Phát triển Organic Flow (Mẫu Q3/2026)
    ...
    Cấu hình Proxy & Playwright  :active,  task3, after task2, 7d

    section 2. Auto & AI
    Kịch bản vượt Captcha        :         task4, after task3, 10d
...
flowchart TD
    ...
    ModalConfig["⚙️ ProxyConfigModal"]:::comp
...
classDiagram
    ...
    class BrowserEngine {
        ...
        +resolveProxy(proxyStr) ProxyConfig
    ...
    BrowserEngine ..> ProxyConfig : Trả về (Returns)
...
flowchart TD
    ...
    F2["Feature 2: Tích hợp Proxy"]:::feature
    ...
    F2 --> T2_1["Task: Code API xoay IP TMProxy"]:::task
    F2 --> T2_2["Task: Xử lý HTTP Proxy Auth"]:::task
    F2 --> T2_3["Task: Inject vào Playwright"]:::task

    %% Rã Task cho Feature 3
    F3 --> T3_1["Task: Logic bắt lỗi Captcha"]:::task
...
pie title Tỷ lệ Các loại Lỗi khi chạy Automation (Tháng 5/2026)
    "Google chặn Captcha" : 45
    "Proxy Timeout / Lỗi mạng" : 30
...
Góc nhìn dành cho **Growth Hacker & Marketing**. Sơ đồ Sankey mô tả "Dòng chảy" (Flow) của một đại lượng. Dưới đây là phễu (Funnel) lưu lượng chạy Bot: Từ 10,000 phiên chạy khởi tạo, bao nhiêu phiên lọt qua được cửa Captcha, bao nhiêu phiên nán lại trang đích thành công để tạo ra Traffic "Thật"?
...
Khoi Tao Profile,Proxy Mang,10000
Proxy Mang,Pass Captcha,7000
Proxy Mang,Timeout Error,3000
Pass Captcha,Trang Dich (Target),6500
Pass Captcha,Loi Keyword,500
...
```mermaid
C4Context
    title C4 Model (Level 1) - System Context cho Organic Flow

    Person(user, "SEO Marketer", "Nhân viên SEO thiết lập chiến dịch và theo dõi kết quả")

    System(bot, "Organic Flow System", "Hệ thống lõi tự động hóa trình duyệt để kéo Traffic")
    ...
    System_Ext(proxy, "Proxy Provider", "Hệ thống đối tác cấp phát IP sạch (Ví dụ: TMProxy)")

    Rel(user, bot, "Tạo chiến dịch & Theo dõi tiến độ", "Desktop App")
    Rel(bot, google, "Gửi lượt Search & Vượt Captcha", "HTTPS / Playwright")
    Rel(bot, target, "Tương tác, cuộn trang > 10s", "HTTPS")
    Rel(bot, proxy, "Lấy IP mới sau mỗi lượt chạy", "REST API")
```
...
kanban
  Todo
    [Tích hợp API Tinsoft Proxy]@{ ticket: ORG-101 }
    [Tính năng Auto-Update]@{ ticket: ORG-102, priority: 'Low' }
  [In progress]
    [Xử lý reCaptcha v3 Google]@{ ticket: ORG-103, assigned: 'Dev1', priority: 'High' }
  Done
    [Thiết kế UI bằng React]@{ ticket: ORG-104, assigned: 'DesignTeam' }
    [Setup Core Playwright]@{ ticket: ORG-105, priority: 'Very High' }
```
...
Góc nhìn dành cho **Solution Architect & Software Engineer**. Đây là **Level 2** của chuẩn C4 Model (Zoom sâu hơn sơ đồ 22). Ở mức này, ta mở hộp đen "Organic Flow System" ra để xem bên trong nó chứa những thùng chứa (Container) nào: Dùng công nghệ gì (React, Node, Prisma, Playwright) và các thùng chứa nói chuyện với nhau ra sao.

```mermaid
C4Container
    title C4 Model (Level 2) - Container Diagram cho Organic Flow

    Person(user, "SEO Marketer", "Người cấu hình và chạy tool")

    System_Boundary(c1, "Organic Flow Desktop App") {
        ...
        ContainerDb(db, "Local SQLite", "Prisma ORM", "Lưu trữ Profile, Proxy, Lịch sử chạy")
        Container(browser, "Headless Browser", "Playwright + Stealth", "Trình duyệt tự động lướt web (Worker)")
    }

    System_Ext(proxy, "TMProxy API", "Hệ thống cung cấp IP")
    ...
    Rel(browser, proxy, "Định tuyến traffic", "HTTP Proxy")
...
flowchart TD
    ...
    subgraph Vulnerabilities ["⚠️ Điểm yếu Hệ thống (Vulnerabilities)"]
        ...
        V2["V2: Lộ API Key Proxy (TMProxy) trên màn hình"]:::vuln
...
flowchart TD
    ...
    subgraph Infrastructure ["🔵 Vòng ngoài: Hạ tầng & Frameworks (Thay đổi liên tục)"]
        ...
        Browser["Playwright Chromium"]:::infra
    end
    ...
    subgraph Domain ["🔴 Lõi trung tâm: Cấu trúc Dữ liệu gốc (Domain Entities)"]
        ProfileEntity["Entity: Profile (ID, Tên, Proxy, RunCount)"]:::domain
    end
...
flowchart TD
    ...
    Start -->|Mở Chrome lên rồi văng ngay| CheckProxy{"Check Proxy/Mạng"}:::question
    Start -->|Chạy giữa chừng thì kẹt lại| CheckCaptcha{"Bị kẹt Captcha?"}:::question
    ...
    CheckProxy -->|Proxy chết/Hết hạn| FixProxy["Mua Proxy mới nhập vào"]:::fix
    CheckProxy -->|Proxy sống| FixBrowser["Xóa thư mục Chromium tải lại"]:::fix

    CheckCaptcha -->|Google hiện Captcha hình ảnh| FixScript["Tăng thời gian Delay giả lập người thật"]:::fix
    CheckCaptcha -->|Trang trắng bóc / Lỗi giao diện| FixSelector["DOM Google đổi, báo Dev update Code"]:::dev
```
...
Góc nhìn dành cho **Product Manager & Strategist**. Đây là sơ đồ biểu diễn các tập hợp giao nhau, chứng minh "Sự kết hợp của 3 yếu tố cốt lõi sẽ tạo ra một Sản phẩm Hoàn hảo (Organic Flow)".

```mermaid
venn-beta
    title Điểm Giao thoa Cốt lõi của Organic Flow
    set Playwright
    set Proxy
    set Profile
    union Playwright,Proxy["Vượt Captcha"]
    union Proxy,Profile["Ẩn danh"]
    union Playwright,Profile["Máy Thật"]
    union Playwright,Proxy,Profile["SEO Master"]
    style Playwright,Proxy fill:#3b82f6
    style Proxy,Profile fill:#f59e0b
    style Playwright,Profile fill:#10b981
    style Playwright,Proxy,Profile fill:#ef4444, color:#fff
```
