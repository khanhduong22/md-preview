(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=document.getElementById(`markdown-editor`),t=document.getElementById(`theme-toggle`),n=document.getElementById(`toggle-sync`),r=document.getElementById(`markdown-editor`),i=document.querySelector(`.preview-pane`),a=document.getElementById(`reading-time`),o=document.getElementById(`word-count`),s=document.getElementById(`char-count`),c=document.getElementById(`toc-toggle-btn`),l=document.getElementById(`sidebar-toc`),u=document.getElementById(`explorer-toggle-btn`),d=document.getElementById(`sidebar-explorer`),f=`markdownViewerTabs`,p=`markdownViewerActiveTab`,m=`markdownViewerUntitledCounter`,h=`markdownViewerGroups`,g=`kido-md-history`,_={tabs:[],activeTabId:null,tabGroups:[],localVaultMode:!1,vaultDirHandle:null,vaultEntries:[],expandedVaultPaths:new Set,vaultMiniSearch:null};async function v(e,t,n){try{if(e.handle.kind===`directory`){let n=await e.parentDir.getDirectoryHandle(t,{create:!0});async function r(e,t){for await(let[n,i]of e.entries())if(i.kind===`file`){let e=await i.getFile(),r=await(await t.getFileHandle(n,{create:!0})).createWritable();await r.write(await e.arrayBuffer()),await r.close()}else i.kind===`directory`&&await r(i,await t.getDirectoryHandle(n,{create:!0}))}await r(e.handle,n),await e.parentDir.removeEntry(e.name,{recursive:!0});let i=_.tabGroups.find(t=>t.name===e.name);i&&(i.name=t,I(_.tabs,_.activeTabId),typeof saveTabGroups==`function`&&saveTabGroups())}else{t.endsWith(`.md`)||(t+=`.md`);let n=await(await e.handle.getFile()).text(),r=await e.parentDir.getFileHandle(t,{create:!0}),i=await r.createWritable();await i.write(n),await i.close(),await e.parentDir.removeEntry(e.name),_.activeTabId===e.path&&(_.activeTabId=e.path.substring(0,e.path.lastIndexOf(`/`))+`/`+t);let a=_.tabs.find(t=>t.id===e.path);a&&(a.id=e.path.substring(0,e.path.lastIndexOf(`/`))+`/`+t,a.title=t,a.handle=r)}await y()}catch(e){alert(`Rename failed: `+e.message),n&&n()}}function ee(e,t,n){let r=document.createElement(`input`);r.type=`text`,r.value=t,r.className=`inline-rename-input`;let i=!1,a=e=>{i||(i=!0,n(e))};r.onblur=()=>a(r.value.trim()),r.onkeydown=e=>{e.key===`Enter`&&a(r.value.trim()),e.key===`Escape`&&a(t)},e.replaceWith(r),r.focus(),r.select()}async function te(t){if(!t)return;if(_.vaultDirHandle=t,_.localVaultMode=!0,document.querySelector(`.empty-vault-message`).style.display=`none`,await _.vaultDirHandle.queryPermission({mode:`readwrite`})!==`granted`){let e=document.createElement(`button`);e.className=`tool-button`,e.style.margin=`10px`,e.innerText=`Re-authorize Vault Access`,e.onclick=async()=>{await _.vaultDirHandle.requestPermission({mode:`readwrite`})===`granted`&&await y()};let t=document.getElementById(`file-tree`);t.innerHTML=``,t.appendChild(e);return}_.tabs=[],_.activeTabId=null,e.value=``,I(_.tabs,_.activeTabId),document.getElementById(`root-new-folder-btn`).style.display=`inline-flex`,document.getElementById(`root-new-file-btn`).style.display=`inline-flex`,document.querySelector(`.vault-action-divider`)&&(document.querySelector(`.vault-action-divider`).style.display=`block`);let n=async()=>{try{let e=prompt(`New Markdown File Name at Root:`);e&&(e.endsWith(`.md`)||(e+=`.md`),await _.vaultDirHandle.getFileHandle(e,{create:!0}),await y())}catch(e){alert(`Error: `+e.message)}},r=async()=>{try{let e=prompt(`New Folder Name at Root:`);e&&(await _.vaultDirHandle.getDirectoryHandle(e,{create:!0}),await y())}catch(e){alert(`Error: `+e.message)}},i=document.getElementById(`root-new-file-btn`),a=document.getElementById(`root-new-folder-btn`);i.replaceWith(i.cloneNode(!0)),a.replaceWith(a.cloneNode(!0)),document.getElementById(`root-new-file-btn`).addEventListener(`click`,n),document.getElementById(`root-new-folder-btn`).addEventListener(`click`,r),await y()}async function ne(e){let t=_.tabs.find(t=>t.id===e.path);if(!t){t=F(``,e.name),t.id=e.path,t.handle=e.handle;let n=e.path.split(`/`).filter(Boolean);if(n.length>1){let e=n[n.length-2],r=_.tabGroups.find(t=>t.name===e);r||=je(e,GROUP_COLORS[Math.floor(Math.random()*GROUP_COLORS.length)].name),t.groupId=r.id}_.tabs.push(t)}await R(t.id)}async function y(){let e=document.getElementById(`file-tree`);e.innerHTML=``,window.MiniSearch&&(_.vaultMiniSearch=new window.MiniSearch({fields:[`title`,`content`],storeFields:[`title`,`content`,`path`]}));let t=async(e,n,r)=>{let i=[];for await(let[t,r]of e.entries())t.startsWith(`.`)||i.push({name:t,handle:r,path:n+`/`+t,parentDir:e});i.sort((e,t)=>e.handle.kind===t.handle.kind?e.name.localeCompare(t.name):e.handle.kind===`directory`?-1:1);for(let e of i){let n=document.createElement(`div`);n.className=`tree-node`+(e.handle.kind===`directory`?` is-dir`:``);let i=`<i class="bi bi-${e.handle.kind===`directory`?`folder`:`file-text`}"></i>`,a=`<span class="tree-node-title" title="${e.name}">${e.name}</span>`,o=`<div class="tree-node-actions">`;e.handle.kind===`directory`&&(o+=`<button class="tree-node-action-btn" title="New File" data-action="new-file"><i class="bi bi-file-earmark-plus"></i></button>`,o+=`<button class="tree-node-action-btn" title="New Folder" data-action="new-folder"><i class="bi bi-folder-plus"></i></button>`),o+=`<button class="tree-node-action-btn" title="Rename" data-action="rename"><i class="bi bi-pencil"></i></button>`,o+=`<button class="tree-node-action-btn" title="Delete" data-action="delete"><i class="bi bi-trash"></i></button>`,o+=`</div>`,n.innerHTML=i+a+o,r.appendChild(n);let s=null;e.handle.kind===`directory`&&(s=document.createElement(`div`),s.className=`tree-children`,s.style.paddingLeft=`16px`,r.appendChild(s),_.expandedVaultPaths.has(e.path)&&(s.classList.add(`expanded`),n.querySelector(`i`).className=`bi bi-folder2-open`),await t(e.handle,e.path,s)),n.onclick=async t=>{let r=t.target.closest(`.tree-node-action-btn`);if(r){t.stopPropagation();let i=r.dataset.action;try{if(i===`new-file`){let t=prompt(`New Markdown File Name:`);t&&(t.endsWith(`.md`)||(t+=`.md`),await e.handle.getFileHandle(t,{create:!0}),_.expandedVaultPaths.add(e.path),await y())}else if(i===`new-folder`){let t=prompt(`New Folder Name:`);t&&(await e.handle.getDirectoryHandle(t,{create:!0}),_.expandedVaultPaths.add(e.path),await y())}else if(i===`delete`){if(confirm(`Are you sure you want to delete "`+e.name+`"?`)){await e.parentDir.removeEntry(e.name,{recursive:!0});let t=_.tabs.find(t=>t.id===e.path);t&&Ve(t.id),await y()}}else if(i===`rename`){let t=n.querySelector(`.tree-node-title`);ee(t,e.name,async n=>{n&&n!==e.name?(r.innerHTML,r.innerHTML=`<i class="bi bi-hourglass-split"></i>`,await v(e,n,()=>{t.replaceWith(t.cloneNode(!0))})):await y()})}}catch(e){alert(`Action failed: `+e.message)}return}t.stopPropagation(),e.handle.kind===`directory`?(s.classList.toggle(`expanded`),s.classList.contains(`expanded`)?_.expandedVaultPaths.add(e.path):_.expandedVaultPaths.delete(e.path),n.querySelector(`i`).className=`bi bi-${s.classList.contains(`expanded`)?`folder2-open`:`folder`}`):e.name.endsWith(`.md`)&&(await ne(e),document.querySelectorAll(`.tree-node`).forEach(e=>e.classList.remove(`active`)),n.classList.add(`active`))};let c=n.querySelector(`.tree-node-title`);c.ondblclick=t=>{t.stopPropagation(),ee(c,e.name,async t=>{t&&t!==e.name?await v(e,t):await y()})},e.handle.kind===`file`&&e.name.endsWith(`.md`)&&e.handle.getFile().then(e=>e.text()).then(t=>{_.vaultMiniSearch&&_.vaultMiniSearch.add({id:e.path,title:e.name,path:e.path,content:t})})}};await t(_.vaultDirHandle,``,e),await re()}async function re(){let e=document.getElementById(`sync-vault-btn`);if(e)if(_.localVaultMode&&_.vaultDirHandle)try{let t=await localforage.getItem(`md-preview-tabs`),n=t?JSON.parse(t):[];n&&n.length>0?(e.style.display=`inline-flex`,e.onclick=async()=>{if(confirm(`Import `+n.length+` virtual notes into your Vault?`))try{for(let e of n){let t=(e.title||`Untitled`).replace(/[\\/:*?"<>|]/g,`-`),n=t+`.md`,r;try{r=await _.vaultDirHandle.getFileHandle(n,{create:!1}),n=t+`_`+Date.now()+`.md`,r=await _.vaultDirHandle.getFileHandle(n,{create:!0})}catch{r=await _.vaultDirHandle.getFileHandle(n,{create:!0})}let i=await r.createWritable();await i.write(e.content||``),await i.close()}alert(`Import successful! Virtual notes are now in your vault.`),await localforage.removeItem(`md-preview-tabs`),e.style.display=`none`,await y()}catch(e){alert(`Error importing notes: `+e.message)}}):e.style.display=`none`}catch{e.style.display=`none`}else e.style.display=`none`}var ie=document.getElementById(`mobile-toggle-sync`),b=document.querySelector(`.content-container`),ae=document.querySelectorAll(`.view-mode-btn:not(.focus-mode-toggle-btn)`),oe=document.querySelectorAll(`.mobile-view-mode-btn`),x=document.querySelector(`.resize-divider`),se=document.querySelector(`.editor-pane`),ce=document.querySelector(`.preview-pane`),S=!0,le=!1,ue=!1,de=null,fe=10,C=`split`,w=!1,T=!1,E=50,D=20;function pe(){!S||ue||(le=!0,clearTimeout(de),de=setTimeout(()=>{let e=r.scrollTop/(r.scrollHeight-r.clientHeight),t=(i.scrollHeight-i.clientHeight)*e;!isNaN(t)&&isFinite(t)&&(i.scrollTop=t),setTimeout(()=>{le=!1},50)},fe))}function me(){!S||le||(ue=!0,clearTimeout(de),de=setTimeout(()=>{let e=i.scrollTop/(i.scrollHeight-i.clientHeight),t=(r.scrollHeight-r.clientHeight)*e;!isNaN(t)&&isFinite(t)&&(r.scrollTop=t),setTimeout(()=>{ue=!1},50)},fe))}function he(){S=!S,S?(n.innerHTML=`<i class="bi bi-link"></i> <span class="btn-label">On</span>`,n.classList.add(`sync-enabled`),n.classList.remove(`sync-disabled`,`border-primary`)):(n.innerHTML=`<i class="bi bi-link-45deg"></i> <span class="btn-label">Off</span>`,n.classList.add(`sync-disabled`,`border-primary`),n.classList.remove(`sync-enabled`))}function O(e){if(!e||![`editor`,`split`,`preview`].includes(e))return;if(e===`split`&&C===`split`){w=!w,b.classList.remove(`view-editor-only`,`view-preview-only`,`view-split`,`view-split-reversed`),b.classList.add(w?`view-split-reversed`:`view-split`),A();return}if(e===C)return;let t=C;C=e,b.classList.remove(`view-editor-only`,`view-preview-only`,`view-split`,`view-split-reversed`),b.classList.add(e===`editor`?`view-editor-only`:e===`preview`?`view-preview-only`:w?`view-split-reversed`:`view-split`),ae.forEach(t=>{t.getAttribute(`data-mode`)===e?(t.classList.add(`active`),t.setAttribute(`aria-pressed`,`true`)):(t.classList.remove(`active`),t.setAttribute(`aria-pressed`,`false`))}),oe.forEach(t=>{t.getAttribute(`data-mode`)===e?(t.classList.add(`active`),t.setAttribute(`aria-pressed`,`true`)):(t.classList.remove(`active`),t.setAttribute(`aria-pressed`,`false`))}),ge(e),e===`split`?A():t===`split`&&Ce(),(e===`split`||e===`preview`)&&U()}function k(e){C=null,O(e||`split`)}function ge(e){let t=e===`split`;n&&(n.style.display=t?``:`none`,n.setAttribute(`aria-hidden`,!t)),ie&&(ie.style.display=t?``:`none`,ie.setAttribute(`aria-hidden`,!t))}function _e(){x&&(x.addEventListener(`mousedown`,ve),document.addEventListener(`mousemove`,be),document.addEventListener(`mouseup`,Se),x.addEventListener(`touchstart`,ye),document.addEventListener(`touchmove`,xe),document.addEventListener(`touchend`,Se))}function ve(e){C===`split`&&(e.preventDefault(),T=!0,x.classList.add(`dragging`),document.body.classList.add(`resizing`))}function ye(e){C===`split`&&(e.preventDefault(),T=!0,x.classList.add(`dragging`),document.body.classList.add(`resizing`))}function be(e){if(!T)return;let t=b.getBoundingClientRect(),n=t.width,r=(e.clientX-t.left)/n*100;r=Math.max(D,Math.min(100-D,r)),E=w?100-r:r,A()}function xe(e){if(!T||!e.touches[0])return;let t=b.getBoundingClientRect(),n=t.width,r=(e.touches[0].clientX-t.left)/n*100;r=Math.max(D,Math.min(100-D,r)),E=w?100-r:r,A()}function Se(){T&&(T=!1,x.classList.remove(`dragging`),document.body.classList.remove(`resizing`))}function A(){if(C!==`split`)return;let e=100-E;se.style.flex=`0 0 calc(${E}% - 4px)`,ce.style.flex=`0 0 calc(${e}% - 4px)`}function Ce(){se.style.flex=``,ce.style.flex=``}function we(){document.querySelectorAll(`.view-mode-btn:not(.focus-mode-toggle-btn)`).forEach(e=>{e.addEventListener(`click`,function(){O(this.getAttribute(`data-mode`)),L()})}),r&&r.addEventListener(`scroll`,pe),i&&i.addEventListener(`scroll`,me),n&&n.addEventListener(`click`,he),_e()}async function Te(){try{let e=await localforage.getItem(f);return e?JSON.parse(e):[]}catch{return[]}}function j(e){if(!_.localVaultMode)try{localforage.setItem(f,JSON.stringify(e))}catch(e){console.warn(`Failed to save AppState.tabs to localforage:`,e)}}async function Ee(){return await localforage.getItem(p)}function M(e){_.localVaultMode||localforage.setItem(p,e)}async function De(){let e=await localforage.getItem(m);return parseInt(e||`0`,10)}function Oe(e){localforage.setItem(m,String(e))}function ke(){return _.untitledCounter=(_.untitledCounter||0)+1,Oe(_.untitledCounter),`Untitled `+_.untitledCounter}var N=[{name:`gray`,bg:`rgba(156,163,175,0.25)`,border:`#9ca3af`,dot:`#9ca3af`},{name:`blue`,bg:`rgba(59,130,246,0.2)`,border:`#3b82f6`,dot:`#3b82f6`},{name:`purple`,bg:`rgba(168,85,247,0.2)`,border:`#a855f7`,dot:`#a855f7`},{name:`green`,bg:`rgba(34,197,94,0.2)`,border:`#22c55e`,dot:`#22c55e`},{name:`yellow`,bg:`rgba(234,179,8,0.25)`,border:`#eab308`,dot:`#eab308`},{name:`orange`,bg:`rgba(249,115,22,0.2)`,border:`#f97316`,dot:`#f97316`},{name:`red`,bg:`rgba(239,68,68,0.2)`,border:`#ef4444`,dot:`#ef4444`},{name:`pink`,bg:`rgba(236,72,153,0.2)`,border:`#ec4899`,dot:`#ec4899`}];async function Ae(){try{let e=await localforage.getItem(h);return e?JSON.parse(e):[]}catch{return[]}}function P(){localforage.setItem(h,JSON.stringify(_.tabGroups))}function je(e,t){let n=N.find(e=>e.name===t)||N[2],r={id:`grp_`+Date.now()+`_`+Math.random().toString(36).slice(2,6),name:e||`New Group`,color:n.name,collapsed:!1};return _.tabGroups.push(r),P(),r}function Me(e){_.tabs.forEach(t=>{t.groupId===e&&(t.groupId=null)}),_.tabGroups=_.tabGroups.filter(t=>t.id!==e),P(),j(_.tabs),I(_.tabs,_.activeTabId)}function Ne(e){let t=_.tabGroups.find(t=>t.id===e);if(!t)return;let n=prompt(`Rename group:`,t.name);n!==null&&(t.name=n.trim()||`Unnamed`,P(),I(_.tabs,_.activeTabId))}function Pe(e){let t=_.tabGroups.find(t=>t.id===e);t&&(t.collapsed=!t.collapsed,P(),I(_.tabs,_.activeTabId))}function Fe(e,t){let n=_.tabGroups.find(t=>t.id===e);n&&(n.color=t,P(),I(_.tabs,_.activeTabId))}function Ie(e,t){let n=_.tabs.find(t=>t.id===e);n&&(n.groupId=t,j(_.tabs),I(_.tabs,_.activeTabId))}function Le(e){let t=_.tabs.find(t=>t.id===e);t&&(t.groupId=null,j(_.tabs),I(_.tabs,_.activeTabId))}function Re(e){return N.find(t=>t.name===e)||N[0]}_.tabGroups=Ae();function F(e,t,n){return e===void 0&&(e=``),t===void 0&&(t=null),n===void 0&&(n=`split`),{id:`tab_`+Date.now()+`_`+Math.random().toString(36).substring(2,8),title:t||`Untitled`,content:e,scrollPos:0,viewMode:n,createdAt:Date.now()}}function I(e,t){let n=document.getElementById(`tab-list`);if(!n)return;n.replaceChildren();function r(e){let t=`<div class="tab-group-submenu">`;return t+=`<button class="tab-menu-item" data-action="new-group"><i class="bi bi-folder-plus"></i> New Group</button>`,_.tabGroups.length>0&&(t+=`<div class="tab-ctx-divider"></div>`,_.tabGroups.forEach(n=>{let r=Re(n.color),i=e===n.id?` ✓`:``;t+=`<button class="tab-menu-item" data-action="add-to-group" data-group-id="`+n.id+`"><span class="group-color-dot" style="background:`+r.dot+`"></span> `+(n.name||`Unnamed`)+i+`</button>`})),e&&(t+=`<div class="tab-ctx-divider"></div>`,t+=`<button class="tab-menu-item" data-action="remove-from-group"><i class="bi bi-folder-minus"></i> Remove from Group</button>`),t+=`</div>`,t}function i(e,n){let i=document.createElement(`div`);i.className=`tab-item`+(e.id===t?` active`:``),n&&(i.style.borderBottomColor=n.border,i.classList.add(`in-group`)),i.setAttribute(`data-tab-id`,e.id),i.setAttribute(`role`,`tab`),i.setAttribute(`aria-selected`,e.id===t?`true`:`false`),i.setAttribute(`draggable`,`true`);let a=document.createElement(`span`);a.className=`tab-title`,a.textContent=e.title||`Untitled`,a.ondblclick=n=>{n.stopPropagation(),startInlineRename(a,e.title||`Untitled`,async n=>{n&&n!==e.title?(_.localVaultMode&&e.handle&&(e.id.split(`/`).pop(),e.handle,e.id,_.vaultDirHandle),e.title=n,j(_.tabs),I(_.tabs,t)):I(_.tabs,t)})},a.title=e.title||`Untitled`;let o=document.createElement(`button`);o.className=`tab-menu-btn`,o.setAttribute(`aria-label`,`File options`),o.title=`File options`,o.textContent=`⋯`;let s=document.createElement(`div`);s.className=`tab-menu-dropdown`,s.replaceChildren(),s.insertAdjacentHTML(`beforeend`,`<button class="tab-menu-item" data-action="pin"><i class="bi bi-pin"></i> <span class="pin-label">`+(e.pinned?`Unpin`:`Pin`)+`</span></button><button class="tab-menu-item" data-action="tag"><i class="bi bi-tag"></i> Tag</button><button class="tab-menu-item tab-menu-item-has-sub" data-action="group-menu"><i class="bi bi-collection"></i> Group <i class="bi bi-chevron-right" style="font-size:10px;margin-left:auto"></i></button><div class="tab-ctx-divider"></div><button class="tab-menu-item" data-action="rename"><i class="bi bi-pencil"></i> Rename</button><button class="tab-menu-item" data-action="duplicate"><i class="bi bi-files"></i> Duplicate</button><button class="tab-menu-item tab-menu-item-danger" data-action="delete"><i class="bi bi-trash"></i> Delete</button>`);let c=document.createElement(`div`);c.className=`tab-group-sub-container`,c.style.display=`none`,c.replaceChildren(),c.insertAdjacentHTML(`beforeend`,r(e.groupId)),s.appendChild(c),o.appendChild(s);let l=s.querySelector(`[data-action="group-menu"]`);return l&&(l.addEventListener(`mouseenter`,()=>{c.style.display=`block`;let e=l.getBoundingClientRect();c.style.top=e.top+`px`,c.style.left=e.left-8+`px`}),l.addEventListener(`click`,e=>{e.stopPropagation(),c.style.display=c.style.display===`none`?`block`:`none`})),s.addEventListener(`mouseleave`,()=>{c.style.display=`none`}),c.querySelectorAll(`.tab-menu-item`).forEach(t=>{t.addEventListener(`click`,n=>{n.stopPropagation(),o.classList.remove(`open`);let r=t.dataset.action;if(r===`new-group`){let t=prompt(`Group name:`);if(t===null)return;let n=je(t,`purple`);Ie(e.id,n.id)}else r===`add-to-group`?Ie(e.id,t.dataset.groupId):r===`remove-from-group`&&Le(e.id)})}),o.addEventListener(`click`,function(e){if(e.stopPropagation(),document.querySelectorAll(`.tab-menu-btn.open`).forEach(function(e){e!==o&&e.classList.remove(`open`)}),o.classList.toggle(`open`),o.classList.contains(`open`)){var t=o.getBoundingClientRect();s.style.top=t.bottom+4+`px`,s.style.right=window.innerWidth-t.right+`px`,s.style.left=`auto`}}),s.querySelectorAll(`:scope > .tab-menu-item`).forEach(function(t){t.addEventListener(`click`,function(n){n.stopPropagation();let r=t.dataset.action;r!==`group-menu`&&(o.classList.remove(`open`),r===`rename`?Ue(e.id):r===`duplicate`?We(e.id):r===`delete`?He(e.id):r===`pin`?Ke(e.id):r===`tag`&&promptTagTab(e.id))})}),i.appendChild(a),i.appendChild(o),i.addEventListener(`click`,function(){R(e.id)}),i.addEventListener(`dragstart`,function(){draggedTabId=e.id,setTimeout(function(){i.classList.add(`dragging`)},0)}),i.addEventListener(`dragend`,function(){i.classList.remove(`dragging`),draggedTabId=null}),i.addEventListener(`dragover`,function(e){e.preventDefault(),i.classList.add(`drag-over`)}),i.addEventListener(`dragleave`,function(){i.classList.remove(`drag-over`)}),i.addEventListener(`drop`,function(t){if(t.preventDefault(),i.classList.remove(`drag-over`),!draggedTabId||draggedTabId===e.id)return;let n=_.tabs.findIndex(e=>e.id===draggedTabId),r=_.tabs.findIndex(t=>t.id===e.id);if(n===-1||r===-1)return;let a=_.tabs.splice(n,1)[0];e.groupId&&(a.groupId=e.groupId),_.tabs.splice(r,0,a),j(_.tabs),I(_.tabs,_.activeTabId)}),i}function a(e){let t=Re(e.color),n=document.createElement(`div`);return n.className=`tab-group-header`+(e.collapsed?` collapsed`:``),n.style.setProperty(`--group-color`,t.border),n.style.setProperty(`--group-bg`,t.bg),n.setAttribute(`data-group-id`,e.id),n.replaceChildren(),n.insertAdjacentHTML(`beforeend`,`<span class="group-color-dot" style="background:`+t.dot+`"></span><span class="group-header-name">`+(e.name||`Unnamed`)+`</span><span class="group-collapse-icon"><i class="bi bi-chevron-`+(e.collapsed?`right`:`down`)+`"></i></span>`),n.addEventListener(`click`,t=>{t.stopPropagation(),Pe(e.id)}),n.addEventListener(`dblclick`,t=>{t.stopPropagation(),Ne(e.id)}),n.addEventListener(`contextmenu`,t=>{t.preventDefault(),t.stopPropagation(),document.querySelectorAll(`.group-context-menu`).forEach(e=>e.remove());let n=document.createElement(`div`);n.className=`group-context-menu`,n.replaceChildren(),n.insertAdjacentHTML(`beforeend`,`<button data-action="rename"><i class="bi bi-pencil"></i> Rename</button><div class="tab-ctx-divider"></div><div class="group-color-picker">`+N.map(t=>`<span class="group-color-option`+(t.name===e.color?` active`:``)+`" data-color="`+t.name+`" style="background:`+t.dot+`"></span>`).join(``)+`</div><div class="tab-ctx-divider"></div><button class="tab-menu-item-danger" data-action="ungroup"><i class="bi bi-folder-minus"></i> Ungroup All</button><button class="tab-menu-item-danger" data-action="delete-group"><i class="bi bi-trash"></i> Delete Group</button>`),n.style.position=`fixed`,n.style.top=t.clientY+`px`,n.style.left=t.clientX+`px`,document.body.appendChild(n),n.querySelectorAll(`button`).forEach(t=>{t.addEventListener(`click`,r=>{r.stopPropagation(),n.remove();let i=t.dataset.action;i===`rename`?Ne(e.id):i===`ungroup`?Me(e.id):i===`delete-group`&&(_.tabs.filter(t=>t.groupId===e.id).forEach(e=>He(e.id)),_.tabGroups=_.tabGroups.filter(t=>t.id!==e.id),P(),I(_.tabs,_.activeTabId))})}),n.querySelectorAll(`.group-color-option`).forEach(t=>{t.addEventListener(`click`,r=>{r.stopPropagation(),n.remove(),Fe(e.id,t.dataset.color)})});let r=()=>{n.remove(),document.removeEventListener(`click`,r)};setTimeout(()=>document.addEventListener(`click`,r),0)}),n.addEventListener(`dragover`,e=>{e.preventDefault(),n.classList.add(`drag-over`)}),n.addEventListener(`dragleave`,()=>{n.classList.remove(`drag-over`)}),n.addEventListener(`drop`,t=>{t.preventDefault(),n.classList.remove(`drag-over`),draggedTabId&&Ie(draggedTabId,e.id)}),n}_.tabGroups.forEach(t=>{let r=e.filter(e=>e.groupId===t.id);if(r.length===0)return;let o=Re(t.color);n.appendChild(a(t)),t.collapsed||r.forEach(e=>{n.appendChild(i(e,o))})}),e.filter(e=>!e.groupId||!_.tabGroups.some(t=>t.id===e.groupId)).forEach(e=>{n.appendChild(i(e,null))});let o=document.createElement(`button`);o.className=`tab-new-btn`,o.title=`New Tab (Ctrl+T)`,o.setAttribute(`aria-label`,`Open new tab`),o.replaceChildren();let s=document.createElement(`i`);s.className=`bi bi-plus-lg`,o.appendChild(s),o.addEventListener(`click`,function(){Be()}),n.appendChild(o);let c=n.querySelector(`.tab-item.active`);c&&c.scrollIntoView({block:`nearest`,inline:`nearest`}),ze(e,t)}function ze(e,t){let n=document.getElementById(`mobile-tab-list`);n&&(n.replaceChildren(),e.forEach(function(e){let r=document.createElement(`div`);r.className=`mobile-tab-item`+(e.id===t?` active`:``),r.setAttribute(`role`,`tab`),r.setAttribute(`aria-selected`,e.id===t?`true`:`false`),r.setAttribute(`data-tab-id`,e.id);let i=document.createElement(`span`);i.className=`mobile-tab-title`,i.textContent=e.title||`Untitled`,i.title=e.title||`Untitled`;let a=document.createElement(`button`);a.className=`tab-menu-btn`,a.setAttribute(`aria-label`,`File options`),a.title=`File options`,a.textContent=`⋯`;let o=document.createElement(`div`);o.className=`tab-menu-dropdown`,o.replaceChildren(),o.insertAdjacentHTML(`beforeend`,`<button class="tab-menu-item" data-action="rename"><i class="bi bi-pencil"></i> Rename</button><button class="tab-menu-item" data-action="duplicate"><i class="bi bi-files"></i> Duplicate</button><button class="tab-menu-item tab-menu-item-danger" data-action="delete"><i class="bi bi-trash"></i> Delete</button>`),a.appendChild(o),a.addEventListener(`click`,function(e){if(e.stopPropagation(),document.querySelectorAll(`.tab-menu-btn.open`).forEach(function(e){e!==a&&e.classList.remove(`open`)}),a.classList.toggle(`open`),a.classList.contains(`open`)){let e=a.getBoundingClientRect();o.style.top=e.bottom+4+`px`,o.style.right=window.innerWidth-e.right+`px`,o.style.left=`auto`}}),o.querySelectorAll(`.tab-menu-item`).forEach(function(t){t.addEventListener(`click`,function(n){n.stopPropagation(),a.classList.remove(`open`);let r=t.getAttribute(`data-action`);r===`rename`?(closeMobileMenu(),Ue(e.id)):r===`duplicate`?(We(e.id),closeMobileMenu()):r===`delete`&&He(e.id)})}),r.appendChild(i),r.appendChild(a),r.addEventListener(`click`,function(){R(e.id),closeMobileMenu()}),n.appendChild(r)}))}document.addEventListener(`click`,function(){document.querySelectorAll(`.tab-menu-btn.open`).forEach(function(e){e.classList.remove(`open`)})});async function L(t=!1){let n=_.tabs.find(function(e){return e.id===_.activeTabId});if(n)if(n.content=e.value,n.scrollPos=e.scrollTop,n.viewMode=C||`split`,n.handle&&t===!0)try{let e=await n.handle.createWritable();if(await e.write(n.content),await e.close(),_.vaultMiniSearch)try{let e={id:n.id,title:n.title,path:n.path,content:n.content};_.vaultMiniSearch.has(n.id)?_.vaultMiniSearch.replace(e):_.vaultMiniSearch.add(e)}catch(e){console.warn(`MiniSearch update error:`,e)}}catch(e){console.error(`Failed to write cleanly to vault:`,e)}else n.handle||j(_.tabs)}async function R(t){if(t===_.activeTabId&&!_.localVaultMode)return;await L(!0),_.activeTabId=t,M(_.activeTabId);let n=_.tabs.find(function(e){return e.id===t});if(n){if(n.handle&&!n.content)try{n.content=await(await n.handle.getFile()).text()}catch{n.content=`Error reading file`}e.value=n.content||``,k(n.viewMode),U(),requestAnimationFrame(function(){e.scrollTop=n.scrollPos||0}),I(_.tabs,_.activeTabId)}}function Be(t,n){t===void 0&&(t=``),n||=ke();let r=F(t,n);r.createdAt=Date.now(),_.tabs.push(r),R(r.id),e.focus()}function Ve(t){let n=_.tabs.findIndex(function(e){return e.id===t});if(n!==-1){if(_.tabs.splice(n,1),_.tabs.length===0){let t=F(``,ke());_.tabs.push(t),_.activeTabId=t.id,M(_.activeTabId),e.value=``,k(`split`),U()}else if(_.activeTabId===t){let t=Math.max(0,n-1);_.activeTabId=_.tabs[t].id,M(_.activeTabId);let r=_.tabs[t];e.value=r.content,k(r.viewMode),U(),requestAnimationFrame(function(){e.scrollTop=r.scrollPos||0})}j(_.tabs),I(_.tabs,_.activeTabId)}}function He(e){Ve(e)}function Ue(e){let t=_.tabs.find(function(t){return t.id===e});if(!t)return;let n=document.getElementById(`rename-modal`),r=document.getElementById(`rename-modal-input`),i=document.getElementById(`rename-modal-confirm`),a=document.getElementById(`rename-modal-cancel`);if(!n||!r)return;r.value=t.title,n.style.display=`flex`,r.focus(),r.select();function o(){let e=r.value.trim();e&&(t.title=e,j(_.tabs),I(_.tabs,_.activeTabId)),n.style.display=`none`,s()}function s(){i.removeEventListener(`click`,o),a.removeEventListener(`click`,c),r.removeEventListener(`keydown`,l)}function c(){n.style.display=`none`,s()}function l(e){e.key===`Enter`?o():e.key===`Escape`&&c()}i.addEventListener(`click`,o),a.addEventListener(`click`,c),r.addEventListener(`keydown`,l)}function We(e){let t=_.tabs.find(function(t){return t.id===e});if(!t)return;L(!0);let n=t.title+` (copy)`,r=F(t.content,n,t.viewMode);t.groupId&&(r.groupId=t.groupId);let i=_.tabs.findIndex(function(t){return t.id===e});_.tabs.splice(i+1,0,r),R(r.id)}function Ge(){let t=document.getElementById(`reset-confirm-modal`),n=document.getElementById(`reset-modal-confirm`),r=document.getElementById(`reset-modal-cancel`),i=document.getElementById(`reset-modal-title`);if(!t)return;_.localVaultMode?(i.textContent=`Close all AppState.tabs? (Local files will NOT be deleted)`,n.textContent=`Close All`):(i.textContent=`Are you sure you want to delete all virtual files and reset to default?`,n.textContent=`Delete & Reset`),t.style.display=`flex`;function a(){if(t.style.display=`none`,s(),_.tabs=[],_.tabGroups=[],P(),_.untitledCounter=0,Oe(0),_.localVaultMode)_.activeTabId=null,e.value=``;else{let t=F(sampleMarkdown,`Welcome to Markdown`);if(_.tabs.push(t),typeof demo30ChartsMarkdown<`u`){let e=je(`demo`,`purple`),t=F(demo30ChartsMarkdown,`30 chart`);t.groupId=e.id,_.tabs.push(t)}_.activeTabId=t.id,e.value=sampleMarkdown}M(_.activeTabId),j(_.tabs),k(`split`),U(),I(_.tabs,_.activeTabId),window.history&&window.history.replaceState?window.history.replaceState(null,null,window.location.pathname):window.location.hash=``}function o(){t.style.display=`none`,s()}function s(){n.removeEventListener(`click`,a),r.removeEventListener(`click`,o)}n.addEventListener(`click`,a),r.addEventListener(`click`,o)}function Ke(e){let t=_.tabs.find(t=>t.id===e);t&&(t.pinned=!t.pinned,j(_.tabs),I(_.tabs,_.activeTabId))}function qe(){let e=document.getElementById(`mobile-menu-panel`),t=document.getElementById(`mobile-menu-overlay`),n=document.getElementById(`mobile-menu-toggle`),r=document.getElementById(`mobile-close-menu`),i=document.getElementById(`mobile-toggle-vault`),a=document.getElementById(`mobile-toggle-sync`),o=document.getElementById(`mobile-import-btn`),s=document.getElementById(`mobile-export-md`),c=document.getElementById(`mobile-export-html`),l=document.getElementById(`mobile-export-pdf`),u=document.getElementById(`mobile-copy-markdown`),d=document.getElementById(`mobile-theme-toggle`),f=document.getElementById(`mobile-new-tab-btn`),p=document.getElementById(`mobile-tab-reset-btn`),m=document.querySelectorAll(`.mobile-view-mode-btn`),h=document.getElementById(`theme-toggle`),g=document.getElementById(`file-input`),_=document.getElementById(`export-md`),v=document.getElementById(`export-html`),ee=document.getElementById(`export-pdf`),te=document.getElementById(`copy-markdown-button`);function ne(){e&&e.classList.add(`active`),t&&t.classList.add(`active`)}function y(){e&&e.classList.remove(`active`),t&&t.classList.remove(`active`)}n&&n.addEventListener(`click`,ne),r&&r.addEventListener(`click`,y),t&&t.addEventListener(`click`,y),i&&i.addEventListener(`click`,()=>{let e=document.getElementById(`sidebar-explorer`);e&&e.classList.toggle(`mobile-active`),y()}),a&&a.addEventListener(`click`,()=>{he(),S?(a.innerHTML=`<i class="bi bi-link me-2"></i> On`,a.classList.add(`sync-enabled`),a.classList.remove(`sync-disabled`,`border-primary`)):(a.innerHTML=`<i class="bi bi-link-45deg me-2"></i> Off`,a.classList.add(`sync-disabled`,`border-primary`),a.classList.remove(`sync-enabled`))}),o&&g&&o.addEventListener(`click`,()=>g.click()),s&&_&&s.addEventListener(`click`,()=>_.click()),c&&v&&c.addEventListener(`click`,()=>v.click()),l&&ee&&l.addEventListener(`click`,()=>ee.click()),u&&te&&u.addEventListener(`click`,()=>te.click()),d&&h&&d.addEventListener(`click`,()=>{h.click(),d.innerHTML=h.innerHTML+` Toggle Dark Mode`}),f&&f.addEventListener(`click`,function(){Be(),y()}),p&&p.addEventListener(`click`,function(){y(),Ge()}),m&&m.forEach(e=>{e.addEventListener(`click`,function(){O(this.getAttribute(`data-mode`)),L(),y()})})}function Je(){let e=document.getElementById(`mobile-char-count`),t=document.getElementById(`mobile-word-count`),n=document.getElementById(`mobile-reading-time`);e&&s&&(e.textContent=s.textContent),t&&o&&(t.textContent=o.textContent),n&&a&&(n.textContent=a.textContent)}function Ye(e){let t=document.createTreeWalker(e,NodeFilter.SHOW_TEXT,null,!1),n=[],r;for(;r=t.nextNode();){let t=r.parentNode,i=!1;for(;t&&t!==e;){if(t.tagName===`PRE`||t.tagName===`CODE`){i=!0;break}t=t.parentNode}!i&&r.nodeValue.includes(`:`)&&n.push(r)}n.forEach(e=>{let t=e.nodeValue,n=/:([\w+-]+):/g,r,i=0,a=``,o=!1;for(;(r=n.exec(t))!==null;){let e=r[1],s=joypixels.shortnameToUnicode(`:${e}:`);s===`:${e}:`?(a+=t.substring(i,n.lastIndex),i=n.lastIndex):(o=!0,a+=t.substring(i,r.index)+s,i=n.lastIndex)}if(o){a+=t.substring(i);let n=document.createElement(`span`);n.innerHTML=a,e.parentNode.replaceChild(n,e)}})}function Xe(e){let t=e.cloneNode(!0),n=e.getBoundingClientRect();t.getAttribute(`width`)||t.setAttribute(`width`,Math.round(n.width)),t.getAttribute(`height`)||t.setAttribute(`height`,Math.round(n.height));let r=new XMLSerializer().serializeToString(t);return`data:image/svg+xml;base64,${btoa(unescape(encodeURIComponent(r)))}`}function Ze(e){return new Promise((t,n)=>{let r=document.createElement(`canvas`),i=r.getContext(`2d`),a=e.getBoundingClientRect(),o=window.devicePixelRatio||2,s=Math.round(a.width),c=Math.round(a.height);r.width=s*o,r.height=c*o,i.scale(o,o),i.fillStyle=getComputedStyle(document.documentElement).getPropertyValue(`--bg-color`).trim()||`#ffffff`,i.fillRect(0,0,s,c);let l=new Image;l.onload=()=>{i.drawImage(l,0,0,s,c),t(r)},l.onerror=n,l.src=Xe(e)})}async function Qe(e,t){let n=e.querySelector(`svg`);if(!n)return;let r=t.innerHTML;t.innerHTML=`<i class="bi bi-hourglass-split"></i>`;try{(await Ze(n)).toBlob(e=>{let n=URL.createObjectURL(e),i=document.createElement(`a`);i.href=n,i.download=`diagram-${Date.now()}.png`,i.click(),URL.revokeObjectURL(n),t.innerHTML=`<i class="bi bi-check-lg"></i>`,setTimeout(()=>{t.innerHTML=r},1500)},`image/png`)}catch(e){console.error(`Mermaid PNG export failed:`,e),t.innerHTML=r}}async function $e(e,t){let n=e.querySelector(`svg`);if(!n)return;let r=t.innerHTML;t.innerHTML=`<i class="bi bi-hourglass-split"></i>`;try{(await Ze(n)).toBlob(async e=>{try{await navigator.clipboard.write([new ClipboardItem({"image/png":e})]),t.innerHTML=`<i class="bi bi-check-lg"></i> Copied!`}catch(e){console.error(`Clipboard write failed:`,e),t.innerHTML=`<i class="bi bi-x-lg"></i>`}setTimeout(()=>{t.innerHTML=r},1800)},`image/png`)}catch(e){console.error(`Mermaid copy failed:`,e),t.innerHTML=r}}function et(e,t){let n=e.querySelector(`svg`);if(!n)return;let r=n.cloneNode(!0),i=new XMLSerializer().serializeToString(r),a=new Blob([i],{type:`image/svg+xml`}),o=URL.createObjectURL(a),s=document.createElement(`a`);s.href=o,s.download=`diagram-${Date.now()}.svg`,s.click(),URL.revokeObjectURL(o);let c=t.innerHTML;t.innerHTML=`<i class="bi bi-check-lg"></i>`,setTimeout(()=>{t.innerHTML=c},1500)}var z=null,B=null;function V(){let e=document.getElementById(`mermaid-zoom-modal`),t=document.getElementById(`mermaid-modal-diagram`);if(!e.classList.contains(`active`))return;e.classList.remove(`active`),B&&=(B.dispose(),null),t.innerHTML=``,z=null;let n=document.querySelector(`.mermaid-modal-content`);n&&n.classList.remove(`fullscreen`);let r=document.getElementById(`mermaid-modal-fullscreen`);r&&(r.innerHTML=`<i class="bi bi-arrows-fullscreen"></i> Fullscreen`)}function tt(e){let t=e.querySelector(`svg`);if(!t)return;let n=document.getElementById(`mermaid-zoom-modal`),r=document.getElementById(`mermaid-modal-diagram`);B&&=(B.dispose(),null),r.innerHTML=``;let i=t.cloneNode(!0);i.removeAttribute(`width`),i.removeAttribute(`height`),i.style.width=`auto`,i.style.height=`auto`,i.style.maxWidth=`100%`,i.style.maxHeight=`100%`,r.appendChild(i),z=i,n.classList.add(`active`),setTimeout(()=>{B=window.panzoom(i,{maxZoom:10,minZoom:.1,smoothScroll:!0,zoomDoubleClickSpeed:1,bounds:!1})},50)}function nt(){let e=document.getElementById(`mermaid-zoom-modal`);document.getElementById(`mermaid-modal-close`).addEventListener(`click`,V),e.addEventListener(`click`,function(t){t.target===e&&V()}),document.getElementById(`mermaid-modal-zoom-in`).addEventListener(`click`,()=>{if(B){let e=B.getTransform();B.smoothZoom(e.x,e.y,1.5)}}),document.getElementById(`mermaid-modal-zoom-out`).addEventListener(`click`,()=>{if(B){let e=B.getTransform();B.smoothZoom(e.x,e.y,.7)}}),document.getElementById(`mermaid-modal-zoom-reset`).addEventListener(`click`,()=>{B&&(B.moveTo(0,0),B.zoomAbs(0,0,1))});let t=document.getElementById(`mermaid-modal-fullscreen`);t&&t.addEventListener(`click`,function(){let e=document.querySelector(`.mermaid-modal-content`);e.classList.toggle(`fullscreen`);let t=e.classList.contains(`fullscreen`);this.innerHTML=t?`<i class="bi bi-fullscreen-exit"></i> Exit`:`<i class="bi bi-arrows-fullscreen"></i> Fullscreen`}),document.getElementById(`mermaid-modal-download-png`).addEventListener(`click`,async function(){if(!z)return;let e=this,t=e.innerHTML;e.innerHTML=`<i class="bi bi-hourglass-split"></i>`;try{(await Ze(z)).toBlob(n=>{let r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=`diagram-${Date.now()}.png`,i.click(),URL.revokeObjectURL(r),e.innerHTML=`<i class="bi bi-check-lg"></i>`,setTimeout(()=>{e.innerHTML=t},1500)},`image/png`)}catch(n){console.error(`Modal PNG export failed:`,n),e.innerHTML=t}}),document.getElementById(`mermaid-modal-copy`).addEventListener(`click`,async function(){if(!z)return;let e=this,t=e.innerHTML;e.innerHTML=`<i class="bi bi-hourglass-split"></i>`;try{(await Ze(z)).toBlob(async n=>{try{await navigator.clipboard.write([new ClipboardItem({"image/png":n})]),e.innerHTML=`<i class="bi bi-check-lg"></i> Copied!`}catch(t){console.error(`Clipboard write failed:`,t),e.innerHTML=`<i class="bi bi-x-lg"></i>`}setTimeout(()=>{e.innerHTML=t},1800)},`image/png`)}catch(n){console.error(`Modal copy failed:`,n),e.innerHTML=t}}),document.getElementById(`mermaid-modal-download-svg`).addEventListener(`click`,function(){if(!z)return;let e=new XMLSerializer().serializeToString(z),t=new Blob([e],{type:`image/svg+xml`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=`diagram-${Date.now()}.svg`,r.click(),URL.revokeObjectURL(n)})}function rt(){let e=document.getElementById(`mermaid-zoom-modal`);document.getElementById(`markdown-preview`).querySelectorAll(`.mermaid-container`).forEach(t=>{if(t.querySelector(`.mermaid-toolbar`)||!t.querySelector(`svg`))return;let n=document.createElement(`div`);n.className=`mermaid-toolbar`,n.setAttribute(`aria-label`,`Diagram actions`);let r=document.createElement(`button`);r.className=`mermaid-toolbar-btn`,r.title=`Zoom diagram`,r.setAttribute(`aria-label`,`Zoom diagram`),r.innerHTML=`<i class="bi bi-arrows-fullscreen"></i>`,r.addEventListener(`click`,()=>tt(t));let i=document.createElement(`button`);i.className=`mermaid-toolbar-btn`,i.title=`Download PNG`,i.setAttribute(`aria-label`,`Download PNG`),i.innerHTML=`<i class="bi bi-file-image"></i> PNG`,i.addEventListener(`click`,()=>Qe(t,i));let a=document.createElement(`button`);a.className=`mermaid-toolbar-btn`,a.title=`Copy image to clipboard`,a.setAttribute(`aria-label`,`Copy image to clipboard`),a.innerHTML=`<i class="bi bi-clipboard-image"></i> Copy`,a.addEventListener(`click`,()=>$e(t,a));let o=document.createElement(`button`);o.className=`mermaid-toolbar-btn`,o.title=`Download SVG`,o.setAttribute(`aria-label`,`Download SVG`),o.innerHTML=`<i class="bi bi-filetype-svg"></i> SVG`,o.addEventListener(`click`,()=>et(t,o)),n.appendChild(r),n.appendChild(a),n.appendChild(i),n.appendChild(o),t.appendChild(n),t.addEventListener(`dblclick`,n=>{n.target.closest(`.mermaid-toolbar`)||(e.classList.contains(`active`)?V():tt(t))})})}function it(){let e=document.documentElement.getAttribute(`data-theme`)===`dark`?`dark`:`default`;mermaid.initialize({startOnLoad:!1,theme:e,securityLevel:`loose`,flowchart:{useMaxWidth:!0,htmlLabels:!0},fontSize:16})}function at(){try{it()}catch(e){console.warn(`Mermaid initialization failed:`,e)}let e={gfm:!0,breaks:!1,pedantic:!1,sanitize:!1,smartypants:!1,xhtml:!1,headerIds:!0,mangle:!1},t=new marked.Renderer;t.code=function(e,t){if(t===`mermaid`){let t=`mermaid-diagram-`+Math.random().toString(36).substr(2,9),n=e;return/^\s*(flowchart|graph)\b/i.test(e)&&(n=e.replace(/(\b\d+\.)\s+/g,`$1&nbsp;`).replace(/(^|[\[\(|]\s*)([-*+])\s+/g,`$1$2&nbsp;`)),`<div class="mermaid-container"><pre class="mermaid" id="${t}">${(e=>e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`))(n)}</pre></div>`}if((t===`plantuml`||t===`puml`)&&window.plantumlEncoder)return`<div class="plantuml-container" style="text-align: center; margin: 1.5em 0;">
...
`),t)}catch(r){return console.warn(`Markdown parsing error, fallback to original parser:`,r),n(e,t)}}}var H=document.getElementById(`markdown-preview`),ot=null,st=150;function U(){try{let t=e.value,n=marked.parse(t);H.innerHTML=DOMPurify.sanitize(n,{ADD_TAGS:[`mjx-container`],ADD_ATTR:[`id`,`class`,`style`,`data-mxgraph`]}),Ye(H),it();try{let e=H.querySelectorAll(`.mermaid`);e.length>0&&(e.forEach(e=>{e.innerHTML.includes(`\\n`)&&(e.innerHTML=e.innerHTML.replace(/\\n/g,`<br/>`))}),mermaid.run({nodes:Array.from(e),suppressErrors:!0}).then(()=>rt()).catch(e=>{console.warn(`Mermaid rendering failed:`,e),rt()}))}catch(e){console.warn(`Mermaid rendering failed:`,e)}if(window.MathJax)try{MathJax.typesetPromise([H]).catch(e=>{console.warn(`MathJax typesetting failed:`,e)})}catch(e){console.warn(`MathJax rendering failed:`,e)}if(window.GraphViewer)try{window.GraphViewer.processElements()}catch(e){console.warn(`Draw.io GraphViewer rendering failed:`,e)}ut(),Je(),ct()}catch(t){console.error(`Markdown rendering failed:`,t),H.innerHTML=`<div class="alert alert-danger">
              <strong>Error rendering markdown:</strong> ${t.message}
          </div>
          <pre>${e.value}</pre>`}}function ct(){let e=document.getElementById(`toc-tree`);if(!e)return;let t=H.querySelectorAll(`h1, h2, h3, h4, h5, h6`);if(t.length===0){e.innerHTML=`<div class="empty-vault-message">No headings found in document.</div>`;return}e.innerHTML=``,t.forEach((t,n)=>{t.id||=t.textContent.toLowerCase().replace(/[^a-z0-9]+/g,`-`).replace(/(^-|-$)+/g,``)||`heading-`+n;let r=parseInt(t.tagName.substring(1)),i=document.createElement(`div`);i.className=`tree-node`,i.style.paddingLeft=`${(r-1)*16+16}px`,r===1?i.style.fontWeight=`600`:r>2&&(i.style.opacity=`0.8`,i.style.fontSize=`0.85rem`),i.innerHTML=`<div class="tree-node-title" title="${t.textContent}">${t.textContent}</div>`,i.addEventListener(`click`,()=>{t.scrollIntoView({behavior:`smooth`})}),e.appendChild(i)})}function lt(){clearTimeout(ot),ot=setTimeout(U,st)}function ut(){let t=e.value,n=t.length;s&&(s.textContent=n.toLocaleString());let r=t.trim()===``?0:t.trim().split(/\s+/).length;o&&(o.textContent=r.toLocaleString());let i=Math.ceil(r/200);a&&(a.textContent=i)}function dt(){let e=window.matchMedia&&window.matchMedia(`(prefers-color-scheme: dark)`).matches;if(document.documentElement.setAttribute(`data-theme`,e?`dark`:`light`),t){t.textContent=``;let n=document.createElement(`i`);n.className=e?`bi bi-sun`:`bi bi-moon`,t.appendChild(n),t.addEventListener(`click`,function(){let e=document.documentElement.getAttribute(`data-theme`)===`dark`?`light`:`dark`;document.documentElement.setAttribute(`data-theme`,e),e===`dark`?t.innerHTML=`<i class="bi bi-sun"></i>`:t.innerHTML=`<i class="bi bi-moon"></i>`,U()})}let n=document.querySelectorAll(`.doc-theme-select`),r=document.getElementById(`markdown-preview`);n.length>0&&r&&n.forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),n.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`);let i=e.getAttribute(`data-theme`);r.className.split(` `).forEach(e=>{e.startsWith(`theme-`)&&r.classList.remove(e)}),i!==`default`&&r.classList.add(`theme-${i}`)})})}function ft(){let e=document.getElementById(`privacy-notice`),t=document.getElementById(`privacy-dismiss`);e&&!localStorage.getItem(`kido-privacy-dismissed`)&&(e.style.display=`block`),t&&t.addEventListener(`click`,()=>{e.classList.add(`dismissing`),localStorage.setItem(`kido-privacy-dismissed`,`1`),setTimeout(()=>{e.style.display=`none`},300)})}var W=[],pt=document.getElementById(`history-btn`),G=document.getElementById(`history-panel`),K=document.getElementById(`history-overlay`),mt=document.getElementById(`history-close`),q=document.getElementById(`history-list`),ht=document.getElementById(`history-diff-view`),gt=document.getElementById(`history-diff-content`),_t=document.getElementById(`history-diff-title`),vt=document.getElementById(`history-back`);async function yt(){try{let e=await localforage.getItem(g);W=e?JSON.parse(e):[]}catch{W=[]}}function J(){return W}function bt(e){W=e,localforage.setItem(g,JSON.stringify(e)).catch(e=>console.warn(e))}function xt(){return Date.now().toString(36)+Math.random().toString(36).slice(2,7)}function St(e,t){let n=J(),r=n.filter(e=>e.tabId===_.activeTabId),i=r.length>0?r[r.length-1]:null,a={id:xt(),tabId:_.activeTabId,title:t||`Untitled`,content:e,timestamp:Date.now(),parentId:i?i.id:null};return n.push(a),n.length>200&&n.splice(0,n.length-200),bt(n),a}function Ct(){if(!G)return;let e=J().filter(e=>!e.tabId||e.tabId===_.activeTabId);ht.style.display=`none`,q.style.display=`block`;let t=_.tabs.find(e=>e.id===_.activeTabId),n=G.querySelector(`h3`);n&&(n.innerHTML=`<i class="bi bi-clock-history"></i> `+(t?t.title:`Version`)+` History`),e.length===0?q.innerHTML=`<div class="history-empty">No history for this note yet.<br>Share it to start tracking versions.</div>`:(q.innerHTML=e.slice().reverse().map(e=>{let t=new Date(e.timestamp).toLocaleString(),n=e.parentId?`<span class="has-parent"><i class="bi bi-git"></i> has parent</span>`:``,r=e.content?e.content.length+` chars`:``;return`<div class="history-item" data-id="`+e.id+`"><div class="history-item-title">`+(e.title||`Untitled`)+`</div><div class="history-item-meta"><span>`+t+`</span><span>`+r+`</span>`+n+`</div></div>`}).join(``),q.querySelectorAll(`.history-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.id;Tt(t)})})),G.style.display=`flex`,K.style.display=`block`}function wt(){G&&(G.style.display=`none`),K&&(K.style.display=`none`)}function Tt(e){let t=J(),n=t.find(t=>t.id===e);if(!n)return;let r=n.parentId?t.find(e=>e.id===n.parentId):null;if(q.style.display=`none`,ht.style.display=`flex`,!r){_t.textContent=`First version — `+(n.title||`Untitled`),gt.innerHTML=n.content.split(`
`).map(e=>`<div class="diff-line diff-added">+ `+Et(e)+`</div>`).join(``);return}_t.textContent=`Changes from previous version`,typeof Diff<`u`&&Diff.diffLines?gt.innerHTML=Diff.diffLines(r.content||``,n.content||``).map(e=>{let t=e.value.split(`
`).filter((e,t,n)=>t<n.length-1||e!==``),n=e.added?`diff-added`:e.removed?`diff-removed`:`diff-unchanged`,r=e.added?`+ `:e.removed?`- `:`  `;return t.map(e=>`<div class="diff-line `+n+`">`+r+Et(e)+`</div>`).join(``)}).join(``):gt.innerHTML=`<div class="history-empty">Diff library not loaded</div>`}function Et(e){let t=document.createElement(`div`);return t.textContent=e,t.innerHTML}function Dt(){pt&&pt.addEventListener(`click`,Ct),mt&&mt.addEventListener(`click`,wt),K&&K.addEventListener(`click`,wt),vt&&vt.addEventListener(`click`,()=>{ht.style.display=`none`,q.style.display=`block`});let e=document.getElementById(`history-resize-handle`);if(e&&G){let t=!1,n=0,r=0;e.addEventListener(`mousedown`,i=>{t=!0,n=i.clientX,r=G.offsetWidth,e.classList.add(`dragging`),document.body.style.cursor=`col-resize`,document.body.style.userSelect=`none`,i.preventDefault()}),document.addEventListener(`mousemove`,e=>{if(!t)return;let i=n-e.clientX,a=Math.min(Math.max(r+i,280),window.innerWidth*.8);G.style.width=a+`px`}),document.addEventListener(`mouseup`,()=>{t&&(t=!1,e.classList.remove(`dragging`),document.body.style.cursor=``,document.body.style.userSelect=``)})}}function Ot(){let t=null,n=null;e.addEventListener(`input`,function(){lt(),clearTimeout(n),n=setTimeout(L,500),clearTimeout(t),t=setTimeout(()=>{let t=e.value;if(!t.trim())return;let n=J().filter(e=>e.tabId===_.activeTabId),r=n.length>0?n[n.length-1]:null;if(!r||Math.abs(t.length-(r.content||``).length)>=20){let e=_.tabs.find(e=>e.id===_.activeTabId);St(t,e?e.title:`Untitled`)}},5e3)}),e.addEventListener(`keydown`,function(e){if(e.key===`Tab`){e.preventDefault();let t=this.selectionStart,n=this.selectionEnd,r=this.value;this.value=r.substring(0,t)+`  `+r.substring(n),this.selectionStart=this.selectionEnd=t+2,this.dispatchEvent(new Event(`input`))}})}var kt=`# Project Master Overview: App Master
...
This is a fully client-side application. Your content never leaves your browser and stays secure on your device.`,jt=32e3;function Mt(e){let t=pako.deflate(new TextEncoder().encode(e)),n=32768,r=``;for(let e=0;e<t.length;e+=n)r+=String.fromCharCode.apply(null,t.subarray(e,e+n));return btoa(r).replace(/\+/g,`-`).replace(/\//g,`_`).replace(/=+$/,``)}function Nt(e){let t=decodeURIComponent(e).replace(/-/g,`+`).replace(/_/g,`/`);for(;t.length%4;)t+=`=`;let n=atob(t),r=Uint8Array.from(n,e=>e.charCodeAt(0));return new TextDecoder().decode(pako.inflate(r))}function Pt(t){let n=_.tabs.find(e=>e.id===_.activeTabId),r=n?n.title:`Untitled`;St(e.value,r);let i=e.value,a;try{a=Mt(i)}catch(e){console.error(`Share encoding failed:`,e),alert(`Failed to encode content for sharing: `+e.message);return}let o=window.location.origin+window.location.pathname+`#share=`+a+`&mode=`+C,s=o.length>jt,c=t.innerHTML;function l(){s||(window.location.hash=`share=`+a+`&mode=`+C),t.innerHTML=`<i class="bi bi-check-lg"></i> Copied!`,setTimeout(()=>{t.innerHTML=c},2e3)}if(navigator.clipboard&&window.isSecureContext)navigator.clipboard.writeText(o).then(l).catch(()=>{});else try{let e=document.createElement(`textarea`);e.value=o,document.body.appendChild(e),e.select(),document.execCommand(`copy`),document.body.removeChild(e),l()}catch{}}function Ft(){let e=document.getElementById(`share-button`),t=document.getElementById(`mobile-share-button`);e&&e.addEventListener(`click`,function(){Pt(e)}),t&&t.addEventListener(`click`,function(){Pt(t)}),window.addEventListener(`hashchange`,()=>{window.location.hash.startsWith(`#share=`)&&Rt()})}function It(){if(typeof pako>`u`)return console.error(`pako is undefined. Cannot load shared content.`),null;let e=``,t=window.location.hash;if(t.startsWith(`#share=`))e=t.slice(7);else{let t=window.location.href.match(/(?:#|%23)share=([^&?]*)/);t&&(e=t[1])}if(!e)return null;let n=e.match(/^[A-Za-z0-9\-_]+/);if(n)e=n[0];else return null;try{return Nt(e)}catch(e){return console.error(`Failed to decode shared content:`,e),alert(`The shared URL could not be decoded. It may be corrupted or incomplete.`),null}}function Lt(){let e=window.location.hash.match(/(?:&|\?)mode=([a-zA-Z]+)/);return e?e[1]:`split`}function Rt(){let t=It();if(t===null)return;let n=F(t,`Shared Note`);_.tabs.push(n),_.activeTabId=n.id,e.value=t,k(Lt()),U(),j(_.tabs),M(_.activeTabId),I(_.tabs,_.activeTabId)}function zt(){let e=document.getElementById(`tab-reset-btn`);e&&e.addEventListener(`click`,function(){Ge()});let t=document.getElementById(`logo-home`);t&&t.addEventListener(`click`,function(){Ge()})}function Bt(){document.addEventListener(`keydown`,async function(t){if((t.ctrlKey||t.metaKey)&&t.key===`s`){t.preventDefault();let n=_.tabs.find(e=>e.id===_.activeTabId);if(_.localVaultMode&&_.vaultDirHandle&&n)if(n.handle){await L(!0);let e=document.getElementById(`exportDropdown`);if(e){let t=e.innerHTML;e.innerHTML=`<i class="bi bi-check2"></i> <span class="btn-label">Saved</span>`,setTimeout(()=>{e.innerHTML=t},1500)}}else try{let t=prompt(`Save to Vault as:`,n.title+`.md`);if(t){t.endsWith(`.md`)||(t+=`.md`);let r;try{if(r=await _.vaultDirHandle.getFileHandle(t,{create:!1}),!confirm(`File "`+t+`" already exists. Overwrite?`))return;r=await _.vaultDirHandle.getFileHandle(t,{create:!0})}catch{r=await _.vaultDirHandle.getFileHandle(t,{create:!0})}let i=await r.createWritable();await i.write(n.content||e.value),await i.close(),n.handle=r,n.title=t.replace(/\.md$/i,``),n.id=`/`+t,I(_.tabs,_.activeTabId),await y();let a=document.getElementById(`exportDropdown`);if(a){let e=a.innerHTML;a.innerHTML=`<i class="bi bi-check2"></i> <span class="btn-label">Saved</span>`,setTimeout(()=>{a.innerHTML=e},1500)}}}catch(e){alert(`Error saving to vault: `+e.message)}else{let e=document.getElementById(`export-md`);e&&e.click()}}if((t.ctrlKey||t.metaKey)&&t.key===`c`){let e=document.activeElement,n=e&&(e.tagName===`TEXTAREA`||e.tagName===`INPUT`),r=window.getSelection&&window.getSelection().toString().trim().length>0;if(!n&&!r){t.preventDefault();let e=document.getElementById(`copy-markdown-button`);e&&e.click()}}(t.ctrlKey||t.metaKey)&&t.shiftKey&&t.key===`S`&&(t.preventDefault(),C===`split`&&he()),(t.ctrlKey||t.metaKey)&&t.key===`t`&&(t.preventDefault(),Be()),(t.ctrlKey||t.metaKey)&&t.key===`w`&&(t.preventDefault(),Ve(_.activeTabId)),t.key===`Escape`&&V()})}function Vt(e){let t=new FileReader;t.onload=function(t){Be(t.target.result,e.name.replace(/\.md$/i,``));let n=document.getElementById(`dropzone`);n&&(n.style.display=`none`)},t.readAsText(e)}var Y=document.getElementById(`dropzone`),X=document.getElementById(`close-dropzone`),Ht=document.getElementById(`file-input`);function Ut(){if(!Y)return;[`dragenter`,`dragover`].forEach(t=>{Y.addEventListener(t,e,!1)}),[`dragleave`,`drop`].forEach(e=>{Y.addEventListener(e,t,!1)});function e(){Y.classList.add(`active`)}function t(){Y.classList.remove(`active`)}Y.addEventListener(`drop`,n,!1),Y.addEventListener(`click`,function(e){X&&e.target!==X&&!X.contains(e.target)&&Ht&&Ht.click()}),X&&X.addEventListener(`click`,function(e){e.stopPropagation(),Y.style.display=`none`});function n(e){let t=e.dataTransfer.files;if(t.length){let e=t[0];e.type===`text/markdown`||e.name.endsWith(`.md`)||e.name.endsWith(`.markdown`)?Vt(e):alert(`Please upload a Markdown file (.md or .markdown)`)}}}var Wt={a4Width:210,a4Height:297,margin:15,contentWidth:180,contentHeight:267,windowWidth:1e3,scale:2};function Gt(e){let t=[];return e.querySelectorAll(`img`).forEach(e=>t.push({element:e,type:`img`})),e.querySelectorAll(`svg`).forEach(e=>t.push({element:e,type:`svg`})),e.querySelectorAll(`pre`).forEach(e=>t.push({element:e,type:`pre`})),e.querySelectorAll(`table`).forEach(e=>t.push({element:e,type:`table`})),t}function Kt(e,t){let n=t.getBoundingClientRect();return e.map(e=>{let t=e.element.getBoundingClientRect(),r=t.top-n.top,i=t.height;return{element:e.element,type:e.type,top:r,height:i,bottom:r+i}})}function qt(e,t,n){let r=t*(n.contentHeight/n.contentWidth),i=[],a=r;for(;a<e;)i.push(a),a+=r;return{boundaries:i,pageHeightPx:r}}function Jt(e,t){if(!e||e.length===0||!t||t.length===0)return[];let n=[];for(let r of e){let e=0;for(let n=0;n<t.length&&r.top>=t[n];n++)e=n+1;let i=0;for(let e=0;e<t.length&&r.bottom>t[e];e++)i=e+1;if(i>e){let i=t[e]||t[0];n.push({element:r.element,type:r.type,top:r.top,height:r.height,splitPageIndex:e,overflowAmount:r.bottom-i})}}return n}function Yt(e){try{let t=Gt(e),n=Kt(t,e),{boundaries:r,pageHeightPx:i}=qt(e.scrollHeight,e.offsetWidth,Wt),a=Jt(n,r);return{totalElements:t.length,splitElements:a,pageCount:r.length+1,pageBoundaries:r,pageHeightPx:i}}catch(e){return console.error(`Page-break analysis failed:`,e),{totalElements:0,splitElements:[],pageCount:1,pageBoundaries:[],pageHeightPx:0}}}var Xt=.3;function Zt(e,t){let n=[],r=[];for(let i of e)i.height<=t?n.push(i):r.push(i);return{fittingElements:n,oversizedElements:r}}function Qt(e,t){for(let n of e){let e=(n.splitPageIndex+1)*t,r=e-n.top;if(r/t>Xt&&n.height*.9<=r)continue;let i=e-n.top+5,a=n.element;n.type===`svg`&&n.element.parentElement&&(a=n.element.parentElement);let o=parseFloat(a.style.marginTop)||0;a.style.marginTop=`${o+i}px`}}function $t(e,t,n=10){let r=0,i,a=-1;do{if(i=Yt(e),!i.pageHeightPx||i.splitElements.length===0||i.splitElements.length===a)break;a=i.splitElements.length;let{fittingElements:t,oversizedElements:n}=Zt(i.splitElements,i.pageHeightPx);if(i.oversizedElements=n,t.length===0)break;Qt(t,i.pageHeightPx),r++}while(r<n);return i}var en=.5;function tn(e,t,n=5){let r=(t-n)/e,i=!1;return r<en&&(r=en,i=!0),{scaleFactor:r,wasClampedToMin:i}}function nn(e,t,n){let r=e.offsetHeight;n===`svg`&&(e.style.maxWidth=`none`),e.style.transform=`scale(${t})`,e.style.transformOrigin=`top left`;let i=r*t;e.style.marginBottom=`-${r-i}px`}function rn(e,t){if(!(!e||e.length===0))for(let n of e){let{scaleFactor:e}=tn(n.height,t);nn(n.element,e,n.type)}}async function an(e,t,n){try{let r=t.innerHTML;t.innerHTML=`<i class="bi bi-hourglass-split"></i> Generating...`,t.disabled=!0;let i=document.createElement(`div`);i.style.position=`fixed`,i.style.top=`50%`,i.style.left=`50%`,i.style.transform=`translate(-50%, -50%)`,i.style.padding=`15px 20px`,i.style.backgroundColor=`rgba(0, 0, 0, 0.7)`,i.style.color=`white`,i.style.borderRadius=`5px`,i.style.zIndex=`9999`,i.style.textAlign=`center`;let a=document.createElement(`div`);a.textContent=`Generating PDF...`,i.appendChild(a),document.body.appendChild(i);let o=window.marked?window.marked.parse(e):marked.parse(e),s=DOMPurify.sanitize(o,{ADD_TAGS:[`mjx-container`,`svg`,`path`,`g`,`marker`,`defs`,`pattern`,`clipPath`],ADD_ATTR:[`id`,`class`,`style`,`viewBox`,`d`,`fill`,`stroke`,`transform`,`marker-end`,`marker-start`]}),c=document.createElement(`div`);c.className=`markdown-body pdf-export`,c.innerHTML=s,c.style.padding=`20px`,c.style.width=`210mm`,c.style.margin=`0 auto`,c.style.fontSize=`14px`,c.style.position=`fixed`,c.style.left=`-9999px`,c.style.top=`0`,c.style.backgroundColor=n===`dark`?`#0d1117`:`#ffffff`,c.style.color=n===`dark`?`#c9d1d9`:`#24292e`,document.body.appendChild(c),await new Promise(e=>setTimeout(e,200));try{window.mermaid&&await window.mermaid.run({nodes:c.querySelectorAll(`.mermaid`),suppressErrors:!0})}catch(e){console.warn(`Mermaid rendering issue:`,e)}if(window.MathJax){try{await MathJax.typesetPromise([c])}catch(e){console.warn(`MathJax rendering issue:`,e)}c.querySelectorAll(`mjx-assistive-mml`).forEach(e=>{e.style.display=`none`,e.style.visibility=`hidden`,e.style.position=`absolute`,e.style.width=`0`,e.style.height=`0`,e.style.overflow=`hidden`,e.remove()}),c.querySelectorAll(`script[type*="math"], script[type*="tex"]`).forEach(e=>e.remove())}await new Promise(e=>setTimeout(e,500));let l=$t(c,Wt);l.oversizedElements&&l.pageHeightPx&&rn(l.oversizedElements,l.pageHeightPx);let u=new window.jspdf.jsPDF({orientation:`portrait`,unit:`mm`,format:`a4`,compress:!0,hotfixes:[`px_scaling`]}),d=u.internal.pageSize.getWidth(),f=u.internal.pageSize.getHeight(),p=d-30,m=await window.html2canvas(c,{scale:2,useCORS:!0,allowTaint:!0,logging:!1,windowWidth:1e3,windowHeight:c.scrollHeight}),h=m.width/p,g=m.height/h,_=Math.ceil(g/(f-30));for(let e=0;e<_;e++){e>0&&u.addPage();let t=e*(f-30)*h,n=Math.min(m.height-t,(f-30)*h),r=n/h,i=document.createElement(`canvas`);i.width=m.width,i.height=n,i.getContext(`2d`).drawImage(m,0,t,m.width,n,0,0,m.width,n);let a=i.toDataURL(`image/png`);u.addImage(a,`PNG`,15,15,p,r)}u.save(`document.pdf`),a.textContent=`Download successful!`,setTimeout(()=>document.body.removeChild(i),1500),document.body.removeChild(c),t.innerHTML=r,t.disabled=!1}catch(e){console.error(`PDF export failed:`,e),alert(`PDF export failed: `+e.message),t.innerHTML=`<i class="bi bi-file-earmark-pdf"></i> Export`,t.disabled=!1;let n=document.querySelector(`div[style*="Generating PDF"]`);n&&document.body.removeChild(n)}}function on(){let t=document.getElementById(`export-md`),n=document.getElementById(`export-html`),r=document.getElementById(`export-pdf`),i=document.getElementById(`copy-markdown-button`);t.addEventListener(`click`,function(){try{let t=new Blob([e.value],{type:`text/markdown;charset=utf-8`});saveAs(t,`document.md`)}catch(e){console.error(`Export failed:`,e),alert(`Export failed: `+e.message)}}),n.addEventListener(`click`,function(){try{let t=e.value,n=marked.parse(t),r=DOMPurify.sanitize(n,{ADD_TAGS:[`mjx-container`],ADD_ATTR:[`id`,`class`,`style`]}),i=document.documentElement.getAttribute(`data-theme`)===`dark`,a=`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
`+i.documentElement.outerHTML,s=new Blob([o],{type:`text/html;charset=utf-8`}),c=URL.createObjectURL(s),l=document.createElement(`a`);l.href=c,l.download=`md-preview.html`,l.click(),URL.revokeObjectURL(c)}catch(e){alert(`Failed to build single file: `+e.message)}finally{e.innerHTML=`<i class="bi bi-file-zip me-1"></i>Download Single File (.html)`}})}}function cn(){yt(),Dt();let e=document.getElementById(`backup-btn`),t=document.getElementById(`restore-btn`),n=document.getElementById(`restore-file-input`);async function r(){let e={version:1,exportedAt:new Date().toISOString(),data:{}},t=[f,p,m,h,g,`kido-privacy-dismissed`];for(let n of t){let t=await localforage.getItem(n);t!==null&&(e.data[n]=t)}let n=new Blob([JSON.stringify(e,null,2)],{type:`application/json`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=`kido-backup-`+new Date().toISOString().slice(0,10)+`.json`,i.click(),URL.revokeObjectURL(r)}function i(e){let t=new FileReader;t.onload=async function(e){try{let t=JSON.parse(e.target.result);if(!t.data||typeof t.data!=`object`){alert(`Invalid backup file format.`);return}if(!confirm(`This will replace all your current notes. Are you sure?`))return;for(let[e,n]of Object.entries(t.data))await localforage.setItem(e,n);alert(`Backup restored successfully! The page will reload.`),location.reload()}catch(e){alert(`Failed to read backup file: `+e.message)}},t.readAsText(e)}e&&e.addEventListener(`click`,e=>{e.preventDefault(),r()}),t&&t.addEventListener(`click`,e=>{e.preventDefault(),n.click()}),n&&n.addEventListener(`change`,e=>{e.target.files.length>0&&i(e.target.files[0]),n.value=``})}function ln(){let e=document.getElementById(`tag-filter-btn`),t=document.getElementById(`tag-filter-dropdown`),n=document.getElementById(`tag-filter-list`),r=document.getElementById(`tag-filter-clear`),i=null;function a(){let e=new Set;return _.tabs.forEach(t=>{(t.tags||[]).forEach(t=>e.add(t))}),Array.from(e).sort()}function o(){if(!n)return;let e=a();if(e.length===0){n.innerHTML=`<div style="padding:8px;color:var(--text-muted);font-size:12px;">No tags yet. Use the tab menu to add tags.</div>`;return}n.innerHTML=e.map(e=>`<div class="tag-filter-item`+(i===e?` active`:``)+`" data-tag="`+e+`"><i class="bi bi-tag"></i> `+e+`</div>`).join(``),n.querySelectorAll(`.tag-filter-item`).forEach(e=>{e.addEventListener(`click`,()=>{i=e.dataset.tag,o(),I(_.tabs,_.activeTabId),t.style.display=`none`})})}e&&e.addEventListener(`click`,()=>{let e=t.style.display!==`none`;t.style.display=e?`none`:`block`,e||o()}),r&&r.addEventListener(`click`,()=>{i=null,I(_.tabs,_.activeTabId),t.style.display=`none`}),document.addEventListener(`click`,e=>{t&&!e.target.closest(`#tag-filter-wrapper`)&&(t.style.display=`none`)})}var un=document.getElementById(`search-btn`),Z=document.getElementById(`search-overlay`),Q=document.getElementById(`search-input`),$=document.getElementById(`search-results`);function dn(){Z&&(Z.style.display=`flex`,setTimeout(()=>Q.focus(),50))}function fn(){Z&&(Z.style.display=`none`,Q.value=``,$.innerHTML=`<div class="search-empty">Type to search across all your notes</div>`)}async function pn(e){if(!e||e.length<2){$.innerHTML=`<div class="search-empty">Type to search across all your notes</div>`;return}let t=[],n=e.toLowerCase();if(t=_.localVaultMode&&_.vaultMiniSearch?_.vaultMiniSearch.search(e,{prefix:!0,fuzzy:.2}).map(e=>({id:e.id,title:e.title||e.id.split(`/`).pop(),content:e.content||``,pinned:!1})):(await Te()).filter(e=>e.title&&e.title.toLowerCase().includes(n)||e.content&&e.content.toLowerCase().includes(n)),t.length===0){$.innerHTML=`<div class="search-empty">No results found</div>`;return}$.innerHTML=t.map(t=>{let r=t.pinned?`<i class="bi bi-star-fill pinned-icon"></i>`:``,i=``;if(t.content){let r=t.content.toLowerCase().indexOf(n);if(r>=0){let n=Math.max(0,r-40),a=Math.min(t.content.length,r+e.length+40),o=t.content.slice(n,r),s=t.content.slice(r,r+e.length),c=t.content.slice(r+e.length,a);i=(n>0?`…`:``)+o+`<mark>`+s+`</mark>`+c+(a<t.content.length?`…`:``)}else i=t.content.slice(0,80)+(t.content.length>80?`…`:``)}return`<div class="search-result-item" data-tab-id="`+t.id+`"><div class="search-result-title">`+r+(t.title||`Untitled`)+`</div><div class="search-result-snippet">`+i+`</div></div>`}).join(``),$.querySelectorAll(`.search-result-item`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.tabId,n=Q?Q.value.trim():``;if(_.localVaultMode){let e=_.vaultEntries?_.vaultEntries.find(e=>e.path===t):null;e&&await ne(e)}else await R(t);fn(),n&&setTimeout(()=>{let e=document.getElementById(`preview-content`);if(!e)return;let t=document.createTreeWalker(e,NodeFilter.SHOW_TEXT),r=n.toLowerCase(),i;for(;i=t.nextNode();){let e=i.nodeValue.toLowerCase().indexOf(r);if(e!==-1){let t=document.createRange();t.setStart(i,e),t.setEnd(i,e+n.length);let r=document.createElement(`mark`);r.className=`search-scroll-highlight`,r.style.cssText=`background:rgba(168,85,247,0.5);border-radius:2px;color:inherit;`,t.surroundContents(r),r.scrollIntoView({behavior:`smooth`,block:`center`}),setTimeout(()=>{let e=r.parentNode;e&&(e.replaceChild(document.createTextNode(r.textContent),r),e.normalize())},2e3);break}}})})})}un&&un.addEventListener(`click`,dn),Z&&Z.addEventListener(`click`,e=>{e.target===Z&&fn()}),Q&&Q.addEventListener(`input`,()=>pn(Q.value)),document.addEventListener(`keydown`,e=>{(e.metaKey||e.ctrlKey)&&e.key===`k`&&(e.preventDefault(),Z&&Z.style.display===`flex`?fn():dn()),e.key===`Escape`&&Z&&Z.style.display===`flex`&&fn()});var mn=`markdownViewerTabs`,hn=`markdownViewerActiveTab`,gn=`markdownViewerUntitledCounter`,_n=`kido-vault-handle`,vn=`markdownViewerGroups`;async function yn(){let e=[mn,hn,gn,vn,`kido-md-history`,`kido-privacy-dismissed`];for(let t of e){let e=localStorage.getItem(t);e!==null&&(await localforage.setItem(t,e),localStorage.removeItem(t))}}async function bn(){await yn(),await yt(),_.tabGroups=await Ae();let t=await localforage.getItem(_n);if(t&&await te(t),_.untitledCounter=await De(),_.tabs=await Te(),_.activeTabId=await Ee(),!_.localVaultMode){if(_.tabs.length===0){let e=F(At,`Welcome to Markdown`);if(_.tabs.push(e),kt!==void 0){let e=F(kt,`30 chart`);_.tabs.push(e)}_.activeTabId=e.id,j(_.tabs),M(_.activeTabId)}else _.tabs.find(e=>e.id===_.activeTabId)||(_.activeTabId=_.tabs[0].id,M(_.activeTabId));let t=It();if(t!==null){let n=F(t,`Shared Note`);_.tabs.push(n),_.activeTabId=n.id,e.value=t,k(Lt()),j(_.tabs),M(_.activeTabId)}else{let t=_.tabs.find(e=>e.id===_.activeTabId);t&&(e.value=t.content||``,k(t.viewMode))}}U(),requestAnimationFrame(()=>{if(!_.localVaultMode){let t=_.tabs.find(e=>e.id===_.activeTabId);t&&(e.scrollTop=t.scrollPos||0)}}),I(_.tabs,_.activeTabId),qe(),zt(),Bt(),Ut(),on(),sn(),cn(),ln(),Ft()}function xn(){if(!window.driver)return;let e=document.getElementById(`privacy-notice`),t=document.getElementById(`privacy-dismiss`);e&&e.style.display!==`none`&&(t?t.click():e.style.display=`none`),C!==`split`&&O(`split`),window.driver.js.driver({showProgress:!0,animate:!0,allowClose:!0,nextBtnText:`Tiếp tục ➔`,prevBtnText:`⬅ Quay lại`,doneBtnText:`Hoàn thành ✔`,steps:[{popover:{title:`Chào mừng đến với 2ndBrain! 🚀`,description:`Chào mừng bạn đến với Markdown Editor cao cấp. Hãy dành 1 phút để lướt qua các tính năng đỉnh cao nhé!`,align:`center`}},{element:`#sidebar-toc`,popover:{title:`Mục lục Tự động (Outline) 📑`,description:`Tự động tạo mục lục dựa trên các thẻ Heading (H1, H2...). Bấm vào để nhảy nhanh tới nội dung.`,side:`right`,align:`start`}},{element:`#open-vault-btn`,popover:{title:`Kết nối Thư mục (Vault) 📂`,description:`Tính năng cực mạnh! Cấp quyền cho trình duyệt đọc/ghi trực tiếp vào một thư mục trên máy tính của bạn. Giống hệt cách Obsidian hoạt động!`,side:`left`,align:`start`}},{element:`#sidebar-explorer`,popover:{title:`Quản lý File Local 🗂️`,description:`Toàn bộ cây thư mục và tài liệu của bạn sẽ hiển thị ở đây. An toàn tuyệt đối, không có bất kỳ dữ liệu nào gửi lên Server lạ.`,side:`left`,align:`start`}},{element:`.tab-bar`,popover:{title:`Tab & Group Tabs 📑`,description:`Mở nhiều file cùng lúc, gom nhóm (Group) màu sắc các Tab lại với nhau siêu gọn gàng y hệt trình duyệt Chrome!`,side:`bottom`,align:`start`}},{element:`#tag-filter-btn`,popover:{title:`Lọc bài viết theo Tag 🏷️`,description:`Chỉ cần gõ #tag vào bài viết, hệ thống sẽ tự động gom nhóm và giúp bạn lọc file cực nhanh ở đây.`,side:`bottom`,align:`end`}},{element:`#tab-reset-btn`,popover:{title:`Reset Toàn bộ 🗑️`,description:`Nếu muốn dọn dẹp sạch sẽ toàn bộ Cache hiện tại để làm lại từ đầu, hãy bấm nút này.`,side:`bottom`,align:`end`}},{element:`.editor-pane`,popover:{title:`Soạn thảo Siêu tốc ⚡`,description:`Gõ Markdown ở đây. Bạn có thể kéo thả ảnh vào hoặc Paste ảnh trực tiếp. Đừng quên dùng phím tắt Ctrl+S để lưu nhé!`,side:`right`,align:`start`}},{element:`.preview-pane`,popover:{title:`Render Thời gian thực 👁️`,description:`Kết quả hiển thị ngay lập tức. Hỗ trợ Toán học (MathJax), Code Highlight (Highlight.js) và tới 30 loại sơ đồ siêu đỉnh (Mermaid.js)!`,side:`left`,align:`start`}},{element:`.view-mode-group`,popover:{title:`Chế độ Hiển thị 🪟`,description:`Chỉ Code, Chia đôi, hoặc Chỉ xem. Bấm nút Hide (mũi tên lên) ẩn thanh công cụ để tập trung tối đa.`,side:`bottom`,align:`center`}},{element:`#toggle-sync`,popover:{title:`Đồng bộ Cuộn trang ↕️`,description:`Tính năng cuộn đồng thời Editor và Preview cực mượt, giúp bạn không bao giờ bị lạc dòng.`,side:`bottom`,align:`start`}},{element:`#import-button`,popover:{title:`Import File 📤`,description:`Nạp nhanh một file Markdown (.md) từ máy tính của bạn vào Editor.`,side:`bottom`,align:`start`}},{element:`#exportDropdown`,popover:{title:`Xuất / Nhập Đa dạng 📥`,description:`Hỗ trợ xuất sang HTML, PDF, Markdown hoặc Backup tải toàn bộ File (JSON) về máy.`,side:`bottom`,align:`start`}},{element:`#copy-markdown-button`,popover:{title:`Copy Markdown 📋`,description:`Copy nhanh toàn bộ nội dung Markdown hiện tại vào Clipboard chỉ với một Click.`,side:`bottom`,align:`start`}},{element:`#share-button`,popover:{title:`Chia sẻ không cần Database 🔗`,description:`Toàn bộ bài viết sẽ được nén lại thành Link. Bạn bè có thể xem ngay lập tức qua mạng mà không cần Database hay Đăng nhập!`,side:`bottom`,align:`end`}},{element:`#search-btn`,popover:{title:`Tìm kiếm Toàn năng 🔍`,description:`Bấm phím tắt Ctrl+K để tìm kiếm nội dung siêu tốc xuyên suốt toàn bộ các file trong Vault của bạn.`,side:`bottom`,align:`end`}},{element:`#history-btn`,popover:{title:`Lịch sử Phiên bản ⏱️`,description:`Lỡ tay xoá nhầm? Đừng lo! Hệ thống tự động lưu lại các mốc thời gian để bạn dễ dàng phục hồi lại bản nháp cũ.`,side:`bottom`,align:`end`}},{element:`#docThemeDropdown`,popover:{title:`Giao diện Tài liệu 🎨`,description:`Thay đổi phong cách văn bản tức thì với các Theme chuyên nghiệp. Hỗ trợ đầy đủ từ Academic, Business đến Hacker.`,side:`bottom`,align:`end`}},{element:`#tour-btn`,popover:{title:`Trợ giúp ❓`,description:`Bạn có thể xem lại hướng dẫn này bất cứ lúc nào bằng nút này. Chúc bạn làm việc hiệu quả!`,side:`left`,align:`center`}}]}).drive()}function Sn(){let e=document.getElementById(`tour-btn`);e&&e.addEventListener(`click`,xn),!localStorage.getItem(`hasSeenTour`)&&window.innerWidth>=768&&setTimeout(()=>{xn(),localStorage.setItem(`hasSeenTour`,`true`)},1e3)}function Cn(){let e=document.getElementById(`focus-mode-btn`),t=document.getElementById(`exit-focus-btn`),n=!1;function r(){n=!n,n?document.body.classList.add(`focus-mode`):document.body.classList.remove(`focus-mode`)}e&&e.addEventListener(`click`,r),t&&t.addEventListener(`click`,r),document.addEventListener(`keydown`,function(e){e.key===`Escape`&&n&&r()})}function wn(){setInterval(async()=>{if(!_.localVaultMode||!_.vaultDirHandle)return;let e=_.tabs.find(e=>e.id===_.activeTabId);if(e){if(e.handle)e.lastVaultSave||=Date.now(),Date.now()-e.lastVaultSave>=9e4&&(await L(!0),e.lastVaultSave=Date.now(),console.log(`Vault file auto-saved`));else if(e.createdAt||=Date.now(),Date.now()-e.createdAt>=3e5)try{let t=e.title.replace(/[\\/:*?"<>|]/g,`-`);t.endsWith(`.md`)||(t+=`.md`);let n;try{n=await _.vaultDirHandle.getFileHandle(t,{create:!1}),t=t.replace(`.md`,`_`+Date.now()+`.md`),n=await _.vaultDirHandle.getFileHandle(t,{create:!0})}catch{n=await _.vaultDirHandle.getFileHandle(t,{create:!0})}e.handle=n,e.id=_.vaultDirHandle.name+`/`+t,e.lastVaultSave=Date.now(),await L(!0),typeof y==`function`&&y(),console.log(`Virtual file auto-saved to vault after 5 minutes`)}catch(e){console.error(`Auto-save failed`,e)}}},3e4)}function Tn(){c&&l&&c.addEventListener(`click`,()=>{l.classList.toggle(`collapsed`)}),u&&d&&u.addEventListener(`click`,()=>{d.classList.toggle(`collapsed`)})}e.value=At,document.addEventListener(`DOMContentLoaded`,()=>{at(),dt(),ft(),Ot(),we(),bn(),wn(),Cn(),Sn(),nt(),Tn()});