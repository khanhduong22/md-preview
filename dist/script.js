...
This is a fully client-side application. Your content never leaves your browser and stays secure on your device.`,et=32e3;function tt(e){let t=pako.deflate(new TextEncoder().encode(e)),n=32768,r=``;for(let e=0;e<t.length;e+=n)r+=String.fromCharCode.apply(null,t.subarray(e,e+n));return btoa(r).replace(/\+/g,`-`).replace(/\//g,`_`).replace(/=+$/,``)}function nt(e){let t=decodeURIComponent(e).replace(/-/g,`+`).replace(/_/g,`/`);for(;t.length%4;)t+=`=`;let n=atob(t),r=Uint8Array.from(n,e=>e.charCodeAt(0));return new TextDecoder().decode(pako.inflate(r))}function rt(t){let n=h.tabs.find(e=>e.id===h.activeTabId),r=n?n.title:`Untitled`;Ze(e.value,r);let i=e.value,a;try{a=tt(i)}catch(e){console.error(`Share encoding failed:`,e),alert(`Failed to encode content for sharing: `+e.message);return}let o=window.location.origin+window.location.pathname+`#share=`+a,s=o.length>et,c=t.innerHTML;function l(){s||(window.location.hash=`share=`+a),t.innerHTML=`<i class="bi bi-check-lg"></i> Copied!`,setTimeout(()=>{t.innerHTML=c},2e3)}if(navigator.clipboard&&window.isSecureContext)navigator.clipboard.writeText(o).then(l).catch(()=>{});else try{let e=document.createElement(`textarea`);e.value=o,document.body.appendChild(e),e.select(),document.execCommand(`copy`),document.body.removeChild(e),l()}catch{}}shareButton.addEventListener(`click`,function(){rt(shareButton)}),mobileShareButton.addEventListener(`click`,function(){rt(mobileShareButton)});function it(){if(typeof pako>`u`)return console.error(`pako is undefined. Cannot load shared content.`),null;let e=``,t=window.location.hash;if(t.startsWith(`#share=`))e=t.slice(7);else{let t=window.location.href.match(/(?:#|%23)share=([^&?]*)/);t&&(e=t[1])}if(!e)return null;let n=e.match(/^[A-Za-z0-9\-_]+/);if(n)e=n[0];else return null;try{return nt(e)}catch(e){return console.error(`Failed to decode shared content:`,e),alert(`The shared URL could not be decoded. It may be corrupted or incomplete.`),null}}function at(){let t=it();if(t===null)return;let n=K(t,`Shared Note`);h.tabs.push(n),h.activeTabId=n.id,e.value=t,Y(`split`),L(),R(h.tabs),z(h.activeTabId),q(h.tabs,h.activeTabId)}window.addEventListener(`hashchange`,()=>{window.location.hash.startsWith(`#share=`)&&at()});var ot=`markdownViewerTabs`,st=`markdownViewerActiveTab`,ct=`markdownViewerUntitledCounter`,lt=`kido-vault-handle`,ut=`markdownViewerGroups`;async function dt(){let e=[ot,st,ct,ut,`kido-md-history`,`kido-privacy-dismissed`];for(let t of e){let e=localStorage.getItem(t);e!==null&&(await localforage.setItem(t,e),localStorage.removeItem(t))}}async function ft(){await dt(),await qe(),h.tabGroups=await Fe();let t=await localforage.getItem(lt);if(t&&await v(t),h.untitledCounter=await Me(),h.tabs=await Ae(),h.activeTabId=await je(),!h.localVaultMode){if(h.tabs.length===0){let e=K($,`Welcome to Markdown`);if(h.tabs.push(e),$e!==void 0){let e=K($e,`30 chart`);h.tabs.push(e)}h.activeTabId=e.id,R(h.tabs),z(h.activeTabId)}else h.tabs.find(e=>e.id===h.activeTabId)||(h.activeTabId=h.tabs[0].id,z(h.activeTabId));let t=it();if(t!==null){let n=K(t,`Shared Note`);h.tabs.push(n),h.activeTabId=n.id,e.value=t,restoreViewMode(`split`),R(h.tabs),z(h.activeTabId)}else{let t=h.tabs.find(e=>e.id===h.activeTabId);t&&(e.value=t.content||``,restoreViewMode(t.viewMode))}}L(),requestAnimationFrame(()=>{if(!h.localVaultMode){let t=h.tabs.find(e=>e.id===h.activeTabId);t&&(e.scrollTop=t.scrollPos||0)}}),q(h.tabs,h.activeTabId),_e()}function pt(){let e=()=>{let e=document.documentElement.getAttribute(`data-theme`)===`dark`?`dark`:`default`;mermaid.initialize({startOnLoad:!1,theme:e,securityLevel:`loose`,flowchart:{useMaxWidth:!0,htmlLabels:!0},fontSize:16})};try{e()}catch(e){console.warn(`Mermaid initialization failed:`,e)}let t={gfm:!0,breaks:!1,pedantic:!1,sanitize:!1,smartypants:!1,xhtml:!1,headerIds:!0,mangle:!1},n=new marked.Renderer;n.code=function(e,t){if(t===`mermaid`){let t=`mermaid-diagram-`+Math.random().toString(36).substr(2,9),n=e;return/^\s*(flowchart|graph)\b/i.test(e)&&(n=e.replace(/(\b\d+\.)\s+/g,`$1&nbsp;`).replace(/(^|[\[\(|]\s*)([-*+])\s+/g,`$1$2&nbsp;`)),`<div class="mermaid-container"><pre class="mermaid" id="${t}">${(e=>e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`))(n)}</pre></div>`}let n=hljs.getLanguage(t)?t:`plaintext`;return`<pre><code class="hljs ${n}">${hljs.highlight(e,{language:n}).value}</code></pre>`},n.link=function(e,t,n){let r=marked.Renderer.prototype.link.call(this,e,t,n);return e&&!e.startsWith(`#`)?r.replace(`<a `,`<a target="_blank" rel="noopener noreferrer" `):r},marked.setOptions({...t,renderer:n});let r=marked.parse;marked.parse=function(e,t){try{let n=e.split(`
